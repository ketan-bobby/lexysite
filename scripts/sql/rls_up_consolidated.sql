...
-- unpolicied. The in-transaction verification block asserts this and
-- aborts (rolls back) if the state does not match.
-- =====================================================================
...
BEGIN
  ...
  IF n_enabled <> n_both OR n_forced <> n_both THEN
    RAISE EXCEPTION 'RLS verification FAILED (rolling back): ENABLE(%)/FORCE(%) not aligned to 42', n_enabled, n_forced;
  END IF;
  IF n_unpol <> 0 THEN
