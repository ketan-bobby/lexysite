/**
 * React hook for streaming a voice response over SSE.
 * Records audio is POSTed as base64, then the server's Server-Sent Events are
 * parsed incrementally into typed events (user_transcript, transcript, audio,
 * error, done), driving live transcript callbacks and AudioWorklet playback.
 * Handles request abort/cancellation, SSE block framing, and final unterminated
 * events. Exports: useVoiceStream.
 */
...
// Split a buffer into complete SSE event blocks (delimited by blank lines),
// returning the trailing partial chunk separately so it can be re-buffered
