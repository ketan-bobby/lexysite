...
const SOURCE_META: Record<
  string,
  { name: string; icon: any; color: string; border: string; desc: string }
> = {
  internal: {
    name: "Talent Pool",
    icon: Users,
    ...
    desc: "Existing candidates in your database",
  },
  github: {
    name: "GitHub",
    icon: Github,
  ...
  pdl: {
    ...
    desc: "500M+ professional profiles",
  },
  serp: {
    name: "Web Search",
    icon: Globe,
    ...
    desc: "Google search of LinkedIn profiles",
  },
  enrichlayer: {
    name: "EnrichLayer",
    icon: Sparkles,
    ...
    desc: "Real LinkedIn profile enrichment",
  },
  linkedin: {
    name: "LinkedIn",
    icon: Linkedin,
...
function MatchBar({ score }: { score: number }) {
  ...
  return (
    <div
      ...
      title="Preliminary fit estimate — refined once the candidate is analysed"
    >
      ...
      <span
        className={`text-sm font-bold ${bandBy(score, { strong: "text-emerald-400", good: "text-primary", fair: "text-orange-400" })}`}
      >
        {score}%
      </span>
...
export default function Sourcing() {
  ...
  /* "all" | "none" (no work order) | a job id — filters the saved sourced pool. */
  const [poolJobFilter, setPoolJobFilter] = useState<string>("all");
  ...
  const { data: status } = useQuery<any>({
    queryKey: ["/api/sourcing/status"],
    queryFn: () => apiFetch("/sourcing/status"),
  });
  const { data: savedData, refetch: refetchSaved } = useQuery<any>({
    queryKey: ["/api/sourcing/candidates"],
    queryFn: () => apiFetch("/sourcing/candidates"),
  });
  ...
  const internalMutation = useMutation({
    mutationFn: (jobId: string) =>
      apiFetch<any>("/sourcing/internal", {
        method: "POST",
  ...
  const searchMutation = useMutation({
    ...
    onSuccess: (data, jobId) => {
      ...
      const failedCount = Object.values((data.queries ?? {}) as Record<string, any>).filter(
        (q: any) => classifySkip(q?.skipped) === "error",
      ).length;
      ...
      toast({
        ...
        description:
          failedCount > 0
            ? `${desc} — ${pluralize(failedCount, "source")} failed (see banner)`
            : desc,
  ...
  const demoMutation = useMutation({
    ...
    onError: (err: any) =>
      toast({
        title: "Could not start demo run",
  ...
  function handleDemoRunDone() {
    ...
    toast({
      title: "Demo run complete",
      description: "Shortlisted candidates added to the pipeline.",
    });
  }
  ...
  const nlMutation = useMutation({
    mutationFn: () =>
      apiFetch<any>("/sourcing/nl-search", {
        method: "POST",
        body: JSON.stringify({
          query: nlQuery.trim(),
    ...
    onSuccess: (data) => {
      ...
      const base =
        data.interpretation ||
        `Internal: ${data.bySource.internal} · External: ${data.bySource.github + data.bySource.pdl + data.bySource.serp + (data.bySource.enrichlayer ?? 0)}`;
      toast({
        title: `Found ${pluralize(data.total, "candidate")}`,
        description:
          failedCount > 0
            ? `${base} — ${pluralize(failedCount, "source")} failed (see banner)`
            : base,
        variant: failedCount > 0 ? "destructive" : undefined,
      });
    },
    onError: (err: any) => {
      ...
      if (String(err?.message ?? "").includes("INTERNAL_REVIEW_REQUIRED")) {
        toast({
          title: "Review your internal talent first",
          description:
            "Switch to the \u201cSelect a Job\u201d tab and run the internal search for this role before sourcing externally.",
          variant: "destructive",
        });
        return;
      }
  ...
  const savedPool = ((savedData as any) || []).map((s: any) => ({
    ...s,
    ...(s.rawData || {}),
    matchScore: s.rawData?.matchScore || Math.round((s.mergeConfidence || 0.5) * 100),
    source: s.source,
  }));
  /* Work-order filter for the saved pool. Each sourced row remembers the job it
   * was sourced FOR (rawData.jobId); "none" = rows sourced without a work order
   * (e.g. NL searches). Fresh search results are already job-scoped, so the
   * filter only applies to the saved-pool view. */
  const poolFiltered =
    poolJobFilter === "all"
      ? savedPool
      : savedPool.filter((c: any) =>
          poolJobFilter === "none" ? !c.jobId : c.jobId === poolJobFilter,
        );
  const displayCandidates = results.length > 0 ? results : poolFiltered;
  const jobTitleById = new Map<string, string>(jobList.map((j: any) => [j.id, j.title]));
    ...
    if (attachedToJob) {
      ...
      return (
        <Button
          size="sm"
          variant="outline"
          className="gap-1.5 h-8 text-xs border-emerald-500/30 text-emerald-400 bg-emerald-500/5"
          disabled
        >
    ...
    return (
      <Select onValueChange={(jobId) => attachMutation.mutate({ candidateId: cid, jobId })}>
        <SelectTrigger className="h-8 w-44 text-xs gap-1.5">
          ...
          <SelectValue placeholder="Add to job…" />
        </SelectTrigger>
        <SelectContent>
          {jobList.length === 0 && (
            <div className="px-2 py-1.5 text-xs text-muted-foreground">No open jobs</div>
          )}
          {jobList.map((j: any) => (
  ...
  return (
    <AppLayout>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h1 data-tour="sourcing-page-title" className="page-title">
            Sourcing Engine
          </h1>
          <p className="text-muted-foreground mt-1">
            AI-powered candidate discovery from your talent pool, GitHub, PDL, web search and more.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3 mb-8">
        {Object.entries(SOURCE_META).map(([id, meta]) => {
          ...
          return (
            <Card
              key={id}
              className={`border ${isLive ? meta.border : "border-white/10"} transition-all`}
            >
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-3">
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center ${isLive ? meta.color : "bg-white/5 text-white/30"}`}
                  >
                  ...
                  <div className="flex items-center gap-1">
                    {isLive ? (
                      ...
                      <>
                        ...
                        <span className="text-[10px] text-white/30 font-semibold">Offline</span>
                      </>
                    )}
                  </div>
                </div>
                ...
                {id === "internal" && (
                  <p className="text-[10px] text-violet-400/80 mt-2">Always available</p>
                )}
                {id === "github" && (
                  <p className="text-[10px] text-emerald-500/80 mt-2">No key required</p>
                )}
                {id !== "internal" && id !== "github" && !hasKey && isLive && (
                  <p className="text-[10px] text-yellow-500/80 mt-2 flex items-center gap-1">
                    <AlertCircle className="w-2.5 h-2.5" /> API key needed
                  </p>
                )}
              </CardContent>
            </Card>
          ...
          <Tabs value={tab} onValueChange={(v) => setTab(v as "job" | "nl")}>
            <TabsList className="mb-4">
              <TabsTrigger value="job" className="gap-2">
                <Briefcase className="w-3.5 h-3.5" /> Select a Job
              </TabsTrigger>
              <TabsTrigger value="nl" className="gap-2">
                <MessageSquare className="w-3.5 h-3.5" /> Describe What You Need
              </TabsTrigger>
            </TabsList>
                ...
                return (
                  <div className="space-y-4">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Internal talent first: review who's already in your own talent pool, then
                          go to market only if you need to. Run the ICP agent first for best
                          results.
                        </p>
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <div
                        className={`rounded-lg border p-4 transition-colors ${internalDone ? "border-violet-500/30 bg-violet-500/5" : "border-primary/20 bg-primary/5"}`}
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <span className="flex items-center justify-center w-5 h-5 rounded-full bg-violet-500/20 text-violet-300 text-[11px] font-bold">
                            1
                          </span>
                        ...
                        <p className="text-xs text-muted-foreground mb-3">
                          Searches only your own talent pool — current employees and candidates
                          already in your database. No external spend, fully private.
                        </p>
                        {internalHasBench && (
                          <p className="text-xs font-medium text-violet-300 mb-3 flex items-center gap-1.5">
                            <Check className="w-3.5 h-3.5" />
                            {pluralize(internalMatchCount ?? 0, "internal match")} found — review
                            below before spending on external.
                          </p>
                        )}
                        {internalEmpty && (
                        ...
                        <div className="relative inline-block">
                          ...
                          <Hotspot
                            id="sourcing-run"
                            tooltip="Pick a job, then review your internal talent first"
                          />
                        </div>
                      </div>

                      {/* Step 2 — external market. Internal review first is a
                          RECOMMENDATION (advisory), never a blocker. */}
                      <div
                        className={`rounded-lg border p-4 transition-colors ${internalEmpty ? "border-primary/40 bg-primary/10" : internalDone ? "border-primary/20 bg-primary/5" : "border-white/10 bg-white/[0.02]"}`}
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <span
                            className={`flex items-center justify-center w-5 h-5 rounded-full text-[11px] font-bold ${internalDone ? "bg-primary/20 text-primary" : "bg-white/10 text-white/40"}`}
                          >
                            2
                          </span>
                        ...
                        <p className="text-xs text-muted-foreground mb-3">
                          {!internalDone
                            ? "Tip: run the free internal search first — you may already have a fit on your bench. You can go straight to market whenever you're ready."
                            : internalEmpty
                        ...
                        <Button
                          ...
                          disabled={!selectedJob || searchMutation.isPending}
                          onClick={() => selectedJob && searchMutation.mutate(selectedJob)}
                          title="Search external sources — uses credits"
                        >
            ...
            <TabsContent value="nl" className="mt-0">
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Type it like you'd ask a colleague. Our AI parses your description, then
                    searches your talent pool and external sources.
                  </p>
                </div>
                <Textarea
                  value={nlQuery}
          ...
          {sourceSummary && (
            <div className="mt-4 pt-4 border-t border-primary/20 flex flex-wrap gap-4 text-sm">
              <span className="text-muted-foreground">
                Total: <strong className="text-foreground">{sourceSummary.total}</strong>
              </span>
              <span className="text-muted-foreground text-violet-400">
                Talent Pool:{" "}
                <strong className="text-violet-300">{sourceSummary.bySource?.internal ?? 0}</strong>
              </span>
              <span className="text-muted-foreground">
                GitHub:{" "}
                <strong className="text-foreground">{sourceSummary.bySource?.github}</strong>
              </span>
              <span className="text-muted-foreground">
                PDL: <strong className="text-foreground">{sourceSummary.bySource?.pdl}</strong>
              </span>
              <span className="text-muted-foreground">
                Web: <strong className="text-foreground">{sourceSummary.bySource?.serp}</strong>
              </span>
              <span className="text-muted-foreground text-fuchsia-300">
                EnrichLayer:{" "}
                <strong className="text-fuchsia-200">
                  {sourceSummary.bySource?.enrichlayer ?? 0}
                </strong>
              </span>
              {sourceSummary.saved > 0 && (
                <span className="text-muted-foreground">
                  Saved: <strong className="text-emerald-400">{sourceSummary.saved}</strong>
                </span>
              )}
            </div>
          )}
      ...
      {demoRunId && <SourcingRunPanel runId={demoRunId} isSimulated onDone={handleDemoRunDone} />}
            ...
            <div className="mb-4 p-4 rounded-xl border border-rose-500/40 bg-rose-500/10 flex items-start gap-3">
              ...
              <div className="text-sm flex-1 min-w-0">
                <p className="font-semibold text-rose-300">
                  {failed.length === 1 ? "1 source failed" : `${failed.length} sources failed`} —
                  these returned 0 because of an error, not because no one matched
                </p>
                ...
                {sourceSummary.parsed && (
                  <div className="flex flex-wrap gap-1.5 mt-2 text-[11px]">
                    ...
                    {sourceSummary.parsed.minYearsExperience != null && (
                      <Badge variant="outline" className="text-[10px]">
                        {sourceSummary.parsed.minYearsExperience}+ yrs
                      </Badge>
                    )}
        ...
        <div className="mb-4 p-4 rounded-xl border border-amber-500/30 bg-amber-500/5 flex items-start gap-3">
          ...
          <div className="text-sm">
            ...
            <p className="text-muted-foreground mt-0.5">
              Sourcing is using only the job title and location. Generate an ICP first to unlock
              alternate titles, certifications, tools, and a domain-aware boolean search.
            </p>
          </div>
        </div>
      )}
            ...
            <div className="grid md:grid-cols-2 gap-3 text-xs">
              {sourceSummary.icpUsed.domain && (
                <div>
                  ...
                  <span className="font-medium ml-1">{sourceSummary.icpUsed.domain}</span>
                  {sourceSummary.icpUsed.subSpecialty ? (
                    <span className="text-muted-foreground">
                      {" "}
                      · {sourceSummary.icpUsed.subSpecialty}
                    </span>
              ...
              {sourceSummary.icpUsed.roleFamily && (
                <div>
                  ...
                  <span className="font-medium ml-1">{sourceSummary.icpUsed.roleFamily}</span>
                </div>
              )}
              {sourceSummary.icpUsed.alternateTitles?.length > 0 && (
                <div className="md:col-span-2">
                  <span className="text-muted-foreground">Alt titles:</span>{" "}
                  <span className="ml-1">
                    {sourceSummary.icpUsed.alternateTitles.slice(0, 6).join(" · ")}
                  </span>
                </div>
              )}
              {sourceSummary.icpUsed.requiredCertifications?.length > 0 && (
                <div className="md:col-span-2">
                  ...
                  <span className="ml-1">
                    {sourceSummary.icpUsed.requiredCertifications.slice(0, 6).join(" · ")}
                  </span>
                </div>
              )}
              {sourceSummary.icpUsed.toolsAndSystems?.length > 0 && (
                <div className="md:col-span-2">
                  <span className="text-muted-foreground">Tools/Systems:</span>{" "}
                  <span className="ml-1">
                    {sourceSummary.icpUsed.toolsAndSystems.slice(0, 6).join(" · ")}
                  </span>
                </div>
              )}
              {sourceSummary.icpUsed.negativeKeywords?.length > 0 && (
                <div className="md:col-span-2">
                  ...
                  <span className="ml-1 text-rose-300">
                    {sourceSummary.icpUsed.negativeKeywords.slice(0, 6).join(" · ")}
                  </span>
                </div>
              )}
              {sourceSummary.icpUsed.booleanSearchString && (
                <div className="md:col-span-2 mt-2 p-2 rounded-md bg-black/30 border border-white/5 font-mono text-[10px] text-emerald-300/90 overflow-x-auto whitespace-pre-wrap break-all">
                  <Code2 className="w-3 h-3 inline mr-1.5 -mt-0.5" />
                  {sourceSummary.icpUsed.booleanSearchString}
                </div>
              )}
            </div>
        ...
        <Card className="mb-6 border-white/10">
          <CardContent className="p-4">
            ...
            <div className="space-y-2">
              {Object.entries(sourceSummary.queries as Record<string, any>).map(
                ([src, q]: [string, any]) => {
                    ...
                    <div key={src} className="flex items-start gap-2 text-[11px]">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {q.skipped ? (
                          <span
                            className={
                              classifySkip(q.skipped) === "error"
                                ? "text-rose-400 italic font-medium"
                          ...
                          <span className="font-mono text-emerald-300/80 break-all">
                            {q.query || "(empty)"}
                          </span>
                        )}
                      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {results.length === 0 && (
              /* Work-order filter — the saved pool spans every job you've sourced
               * for, so let the recruiter narrow it to one work order. */
              <Select value={poolJobFilter} onValueChange={setPoolJobFilter}>
        ...
        <CardContent>
          {displayCandidates.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {tab === "nl"
                  ? "Describe the candidate you're looking for, then click Source Now."
                  : "Select a job and click Source Now to search your talent pool, GitHub, PDL and the web."}
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {displayCandidates.map((c: any, i: number) => {
                ...
                return (
                  <div
                    key={c.id || i}
                    className={`flex items-center justify-between p-4 rounded-xl border transition-all group ${isInternal ? "border-violet-500/25 bg-violet-500/5 hover:border-violet-500/50" : "border-border/50 hover:border-primary/30"}`}
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm shrink-0 ${isInternal ? "bg-violet-500/20 text-violet-300" : "bg-primary/10 text-primary"}`}
                      >
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                          ...
                          <Badge
                            variant="outline"
                            className={`text-[10px] gap-1 py-0 ${isInternal ? "border-violet-500/30 text-violet-400 bg-violet-500/10" : ""}`}
                          >
                            <Icon className="w-2.5 h-2.5" /> {src?.name || c.source}
                          </Badge>
                          {c.jobId && jobTitleById.get(c.jobId) && (
                            <Badge
                              variant="outline"
                              className="text-[10px] py-0 border-sky-500/30 text-sky-400 bg-sky-500/10"
                              title="Work order this candidate was sourced for"
                            >
                              {jobTitleById.get(c.jobId)}
                            </Badge>
                          )}
                          {isInternal && !c.isCurrentEmployee && (
                            <Badge
                              variant="outline"
                              className="text-[10px] py-0 border-violet-500/30 text-violet-400 bg-violet-500/10"
                            >
                              In your DB
                            </Badge>
                          )}
                          {c.isCurrentEmployee && (
                            <Badge
                              variant="outline"
                              className="text-[10px] py-0 border-emerald-500/40 text-emerald-300 bg-emerald-500/10 gap-1"
                            >
                        ...
                        <p className="text-sm text-muted-foreground flex items-center gap-2 flex-wrap">
                          ...
                          {c.locationFlag && (
                            <Badge
                              variant="outline"
                              className="text-[10px] py-0 gap-1 border-amber-500/40 text-amber-400 bg-amber-500/10"
                            >
                        ...
                        {c.matchReason && (
                          <p
                            className={`text-xs mt-2 leading-snug ${isInternal ? "text-violet-200/80" : "text-muted-foreground"}`}
                          >
      ...
      {!connectors.pdl?.apiKey && (
        <div className="mt-4 p-4 rounded-xl border border-yellow-500/20 bg-yellow-500/5 flex items-start gap-3">
          ...
          <div className="text-sm">
            ...
            <p className="text-muted-foreground mt-0.5">
              Add your <code className="text-xs bg-white/10 px-1 rounded">PDL_API_KEY</code>{" "}
              environment variable to unlock People Data Labs (500M+ profiles). Your talent pool,
              GitHub and web search are active in the meantime.
            </p>
          </div>
        </div>
      )}
