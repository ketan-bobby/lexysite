...
import {
  Loader2, Github, Linkedin, Sparkles, MapPin, Users, ArrowRight,
  UserPlus, Mail, Video, ExternalLink, Upload, FileText, Download,
  CheckCircle2, XCircle, AlertTriangle, ThumbsUp, Calendar, MessageSquare,
  ShieldCheck, ShieldAlert, ShieldQuestion, Plus, Trash2, Save, Check, Target,
  DollarSign, UserCheck, ThumbsDown, Play, Send, Copy,
  Maximize2, Minimize2, MoreHorizontal, RotateCcw,
} from "lucide-react";
import {
...
import {
  OutreachDraftCard, useJobPendingDrafts, type OutreachMessage,
...
async function apiFetch<T>(path: string, opts?: RequestInit): Promise<T> {
  ...
  if (!res.ok) {
    ...
    } catch { /* response wasn't JSON, fall back to status */ }
...
const STAGE_PHASE: Record<string, Phase> = {
  sourced: "sourcing", screening: "sourcing", verification: "sourcing",
  shortlisted: "engagement", interview: "engagement",
  interview_scheduled: "engagement", interview_completed: "engagement",
  hm_review: "decision", offer: "decision", offer_recommended: "decision",
  offer_extended: "decision", offer_accepted: "decision",
  hired: "decision", started: "decision",
};
...
type PhaseStyle = {
  label: string; text: string; header: string; pill: string;
  stripText: string; stripCount: string; stripBg: string; stripBorder: string;
};
const PHASE_STYLE: Record<Phase, PhaseStyle> = {
  sourcing:   { label: "Sourcing",   text: "text-muted-foreground", header: "text-muted-foreground border-border/60",                    pill: "bg-muted text-muted-foreground",     stripText: "text-muted-foreground/70", stripCount: "text-muted-foreground/50", stripBg: "bg-muted/30",           stripBorder: "border-border/40" },
  engagement: { label: "Engagement", text: "text-primary",          header: "text-primary border-primary/20 bg-primary/5",               pill: "bg-primary/15 text-primary",         stripText: "text-primary/60",          stripCount: "text-primary/50",          stripBg: "bg-primary/[0.04]",     stripBorder: "border-primary/15" },
  decision:   { label: "Decision",   text: "text-signal-green",     header: "text-signal-green border-signal-green/20 bg-signal-green/5", pill: "bg-signal-green/15 text-signal-green", stripText: "text-signal-green/60",     stripCount: "text-signal-green/50",     stripBg: "bg-signal-green/[0.04]", stripBorder: "border-signal-green/15" },
};
/* Neutral fallback for terminal-negative / unmapped columns (pre-Step-3). */
const NEUTRAL_COL: Omit<PhaseStyle, "label"> = { text: "text-muted-foreground", header: "text-muted-foreground border-border/60", pill: "bg-muted text-muted-foreground", stripText: "text-muted-foreground/70", stripCount: "text-muted-foreground/50", stripBg: "bg-muted/30", stripBorder: "border-border/40" };

const NBA_LABELS: Record<string, { label: string; color: string }> = {
  advance:             { label: "Advance",   color: "text-emerald-700 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/20" },
  schedule:            { label: "Schedule",  color: "text-blue-700 dark:text-blue-400 bg-blue-500/10 border-blue-500/20" },
  recruiter_review:    { label: "Review",    color: "text-amber-700 dark:text-yellow-400 bg-amber-500/10 dark:bg-yellow-500/10 border-amber-500/20 dark:border-yellow-500/20" },
  re_engage:           { label: "Re-Engage", color: "text-orange-700 dark:text-orange-400 bg-orange-500/10 border-orange-500/20" },
  manual_verification: { label: "Verify",    color: "text-violet-700 dark:text-violet-400 bg-violet-500/10 border-violet-500/20" },
  reject:              { label: "Reject",    color: "text-rose-700 dark:text-rose-400 bg-rose-500/10 border-rose-500/20" },
  hold:                { label: "Hold",      color: "text-slate-600 dark:text-slate-400 bg-slate-500/10 border-slate-500/20" },
};
...
function ScorePill({ score, label = "Match" }: { score: number | null; label?: string }) {
  ...
  return <span title={`${label} score`} className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border whitespace-nowrap", color)}>{label} {score}%</span>;
}
...
function PrimaryLink({ label, icon: Icon, onClick, disabled, loading, title, tone = "primary" }: {
  label: string; icon?: any; onClick: (e: React.MouseEvent) => void;
  disabled?: boolean; loading?: boolean; title?: string;
  tone?: "primary" | "success" | "danger";
}) {
  const color = tone === "danger" ? "text-rose-600 dark:text-rose-400"
    : tone === "success" ? "text-signal-green" : "text-primary";
  return (
    <button
      type="button"
      title={title}
      ...
      className={cn(
        ...
        color, !(disabled || loading) && "hover:opacity-70",
      )}
    >
...
/* ── Candidate Card ───────────────────────────────────────────────────── */
function CandidateCard({ row, jobId, onMove, draft, restoreToBoard }: { row: any; jobId: string; onMove: () => void; draft?: OutreachMessage; restoreToBoard?: boolean }) {
  ...
  const copyInterviewLink = async (sessionId: string | null | undefined) => {
    if (!sessionId) {
      toast({ title: "No interview link yet", description: "Schedule the interview first — no session exists for this candidate.", variant: "destructive" });
      return;
    }
    const roomUrl = `${window.location.origin + BASE}/interviews/${sessionId}/room`;
    try {
      await navigator.clipboard.writeText(roomUrl);
      toast({ title: "Interview link copied!", description: "Paste it into your email to the candidate." });
    } catch {
  ...
  const advanceMutation = useMutation({
    mutationFn: async () => {
      ...
      const NEXT: Record<string, string> = {
        applied: "screening", screening: "verification",
        verification: "shortlisted", shortlisted: "interview_scheduled",
    ...
    onSuccess: () => {
      ...
      if (row.stage === "screening") {
        toast({ title: "Running verification…", description: "The Verification Agent is checking this candidate." });
      ...
      if (row.stage === "verification") {
        toast({
          ...
          description: "Lexy is writing the first-touch email. It'll wait on this job's Outreach tab for your review before sending.",
          action: (
            <ToastAction altText="Review draft" onClick={() => setLocation(`/jobs/${jobId}?tab=outreach`)}>
              Review draft
            </ToastAction>
          ),
        });
    ...
    onError: (err: any) => {
      ...
      if (err?.code === "INTERVIEW_REQUIRED" && row.applicationId) {
        toast({
          ...
          description: "This candidate has no completed interview. You can override and move them to Offer anyway.",
          variant: "destructive",
          action: (
            <ToastAction
              ...
              onClick={async () => {
                try {
                  ...
                  qc.invalidateQueries({ queryKey: ["pipeline-stages", jobId] });
                  onMove();
                  toast({ title: "Moved to Offer", description: "Interview requirement overridden." });
                } catch (e: any) {
  ...
  const moveWithoutAutomationMutation = useMutation({
    mutationFn: async (targetStage: string) => {
      if (!row.applicationId) {
        return apiFetch(`/applications`, {
          method: "POST",
          body: JSON.stringify({ jobId, candidateId: c.id, stage: targetStage, skipAutomation: true }),
        });
      }
      return apiFetch(`/applications/${row.applicationId}`, {
        method: "PUT",
    ...
    onSuccess: (_data, targetStage) => {
      ...
      toast({ title: `Moved to ${label}`, description: "No automated emails or agents were triggered." });
      qc.invalidateQueries({ queryKey: ["pipeline-stages", jobId] });
      onMove();
    },
  ...
  const verifyMutation = useMutation({
    mutationFn: () => apiFetch<any>(`/jobs/${jobId}/pipeline/card-action`, {
      method: "POST",
  ...
  const outreachMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (data?.pendingApproval) {
        toast({
          ...
          description: "The initial outreach email was drafted. Review and approve it on this job's Outreach tab to send it.",
          action: (
            <ToastAction altText="Review draft" onClick={() => setLocation(`/jobs/${jobId}?tab=outreach`)}>
              Review draft
            </ToastAction>
    ...
    onError: (err: any) => {
      toast({
        ...
        description: err?.message || "Could not deliver outreach email. Check API logs for details.",
        variant: "destructive",
      });
    },
  });
  ...
  const acceptMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        action: canCopy ? (
          <ToastAction altText="Copy interview link" onClick={() => copyInterviewLink(data.sessionId)}>
            Copy link
          </ToastAction>
        ) : undefined,
      });
    ...
    onError: (err: any) => toast({ title: "Failed to send interview invite", description: err?.message, variant: "destructive" }),
  });
  ...
  const generateLinkMutation = useMutation({
    mutationFn: () => apiFetch<any>("/interviews/generate-link", {
      method: "POST",
      body: JSON.stringify({
        jobId,
  ...
  const copyOrGenerateLink = () => {
    if (c?.interviewSessionId) { copyInterviewLink(c.interviewSessionId); return; }
    if (!c?.id && !row.applicationId) {
      toast({ title: "No interview link yet", description: "This candidate can't be linked to an interview yet.", variant: "destructive" });
      return;
    }
  ...
  const extendOfferMutation = useMutation({
    mutationFn: () => apiFetch<any>(`/outcomes/${row.applicationId}/extend-offer`, {
      method: "PUT",
      body: JSON.stringify({}),
    }),
    onSuccess: () => {
    ...
    onError: (err: any) => toast({ title: "Failed to extend offer", description: err?.message, variant: "destructive" }),
  });

  const acceptOfferMutation = useMutation({
    mutationFn: () => apiFetch<any>(`/outcomes/${row.applicationId}/accept-offer`, {
      method: "PUT",
      body: JSON.stringify({}),
    }),
    onSuccess: () => {
    ...
    onError: (err: any) => toast({ title: "Failed to mark accepted", description: err?.message, variant: "destructive" }),
  });

  const declineOfferMutation = useMutation({
    mutationFn: () => apiFetch<any>(`/outcomes/${row.applicationId}/decline-offer`, {
      method: "PUT",
      body: JSON.stringify({}),
    }),
    onSuccess: () => {
    ...
    onError: (err: any) => toast({ title: "Failed to mark declined", description: err?.message, variant: "destructive" }),
  });

  const hireMutation = useMutation({
    mutationFn: () => apiFetch<any>(`/outcomes/${row.applicationId}/hire`, {
      method: "PUT",
      body: JSON.stringify({}),
    }),
    onSuccess: () => {
    ...
    onError: (err: any) => toast({ title: "Failed to mark hired", description: err?.message, variant: "destructive" }),
  });

  const startMutation = useMutation({
    mutationFn: () => apiFetch<any>(`/outcomes/${row.applicationId}/start`, {
      method: "PUT",
      body: JSON.stringify({}),
    }),
    onSuccess: () => {
    ...
    onError: (err: any) => toast({ title: "Failed to mark started", description: err?.message, variant: "destructive" }),
  });

  const [replyOpen, setReplyOpen] = useState(false);
  const [rejectOpen, setRejectOpen] = useState(false);
  const isPending = advanceMutation.isPending || verifyMutation.isPending || outreachMutation.isPending || acceptMutation.isPending || sendToHmMutation.isPending
    || extendOfferMutation.isPending || acceptOfferMutation.isPending || declineOfferMutation.isPending || hireMutation.isPending || startMutation.isPending;

  return (
    <>
    <CandidateDetailDialog row={row} jobId={jobId} open={detailOpen} onClose={() => setDetailOpen(false)} />
    <QuickReplyDialog row={row} jobId={jobId} open={replyOpen} onClose={() => setReplyOpen(false)} />
    <RejectCandidateDialog row={row} jobId={jobId} open={rejectOpen} onClose={() => setRejectOpen(false)} />
    {draft && (
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-start gap-2.5">
          ...
          <div className="flex-1 min-w-0">
            {(() => {
              const name = (c?.firstName || c?.lastName) ? `${c?.firstName ?? ""} ${c?.lastName ?? ""}`.trim() : (c?.email || "Unknown candidate");
              return <p title={name} className="text-sm font-semibold truncate leading-tight text-foreground">{name}</p>;
            })()}
            {(() => {
              const sub = [c?.currentTitle, c?.location || c?.currentCompany].filter(Boolean).join(" · ");
              return sub ? <p title={sub} className="text-xs text-muted-foreground truncate mt-0.5">{sub}</p> : null;
            })()}
            ...
            {(() => {
              ...
              const label = (c as any)?.locationTierLabel ?? (c as any)?.rawData?.locationTierLabel;
              if (!tier || tier === "city" || tier === "remote" || !label) return null;
              return (
                <div className="text-[10px] text-amber-600 dark:text-amber-400 flex items-center gap-1 min-w-0" title={String(label)}>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
              <Button size="icon" variant="ghost" aria-label="More actions" className="w-7 h-7 -mr-1.5 -mt-1 shrink-0 text-muted-foreground hover:text-foreground">
            ...
            <DropdownMenuContent align="end" className="w-56" onClick={(e) => e.stopPropagation()}>
                  ...
                  <DropdownMenuSub>
                    ...
                    <DropdownMenuSubContent>
                      {RESTORE_STAGES.map((st) => {
                        ...
                        const dot = ph === "engagement" ? "bg-primary" : ph === "decision" ? "bg-signal-green" : "bg-muted-foreground/50";
                        return (
                          <DropdownMenuItem key={st.key} className="text-xs" disabled={moveWithoutAutomationMutation.isPending} onSelect={() => moveWithoutAutomationMutation.mutate(st.key)}>
              ...
              {row.candidate?.interviewSessionId && (
                <DropdownMenuItem className="text-xs" onSelect={() => setLocation(`/interviews/${row.candidate!.interviewSessionId}`)}>
                  <FileText className="w-3.5 h-3.5 mr-2" /> Interview report
                </DropdownMenuItem>
              )}
              {(row.stage === "interview" || row.stage === "interview_scheduled") && row.candidate?.interviewSessionId && (
              ...
              {row.stage === "shortlisted" && row.sourcedId && draft && (
                <DropdownMenuItem className="text-xs" disabled={isPending} onSelect={() => acceptMutation.mutate()}>
                  <Video className="w-3.5 h-3.5 mr-2" /> Move to interview
                </DropdownMenuItem>
              )}
              {(row.stage === "interview_completed" || row.stage === "hm_review") && (
                <DropdownMenuItem className="text-xs" asChild>
                  <a href="/coordinator"><Calendar className="w-3.5 h-3.5 mr-2" /> Schedule interview</a>
                </DropdownMenuItem>
              )}
              {(row.stage === "interview_completed" || row.stage === "hm_review") && hmCandidateId && (
                <DropdownMenuItem className="text-xs" onSelect={() => setLocation(`/candidates/${hmCandidateId}?hmShare=1`)}>
                  <Send className="w-3.5 h-3.5 mr-2" /> Send packet to HM
                </DropdownMenuItem>
              )}
              {row.stage === "offer_extended" && row.applicationId && (
                <DropdownMenuItem className="text-xs" onSelect={() => declineOfferMutation.mutate()}>
              ...
              {c?.githubProfile && (
                <DropdownMenuItem className="text-xs" asChild>
                  <a href={c.githubProfile} target="_blank" rel="noopener noreferrer"><Github className="w-3.5 h-3.5 mr-2" /> GitHub profile</a>
                </DropdownMenuItem>
              )}
              {c?.linkedinUrl && (
                <DropdownMenuItem className="text-xs" asChild>
                  <a href={c.linkedinUrl.startsWith("http") ? c.linkedinUrl : `https://${c.linkedinUrl}`} target="_blank" rel="noopener noreferrer"><Linkedin className="w-3.5 h-3.5 mr-2" /> LinkedIn profile</a>
                </DropdownMenuItem>
              )}
              <DropdownMenuSeparator />
              <DropdownMenuLabel className="text-xs">
                Move to stage
                <span className="block font-normal text-muted-foreground">No emails or agents triggered</span>
              </DropdownMenuLabel>
              {STAGE_COLS.filter((st) => st.key !== row.stage && st.key !== "sourced").map((st) => {
                const ph = STAGE_PHASE[st.key];
                const dot = ph === "engagement" ? "bg-primary" : ph === "decision" ? "bg-signal-green" : "bg-muted-foreground/50";
                return (
                  <DropdownMenuItem key={st.key} className="text-xs" onSelect={() => moveWithoutAutomationMutation.mutate(st.key)}>
                    <span className={cn("w-1.5 h-1.5 rounded-full mr-2 shrink-0", dot)} />
                    {st.label}
                  </DropdownMenuItem>
                );
              ...
              {!isTerminal && row.stage !== "offer_accepted" && (
                <>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem className="text-xs text-rose-600 focus:text-rose-600 dark:text-rose-400 dark:focus:text-rose-400" onSelect={() => setRejectOpen(true)}>
          ...
          return (
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {(src === "manual" || src === "manual_import") && (
                <span className="text-[9px] px-2 py-0.5 rounded-full bg-violet-500/10 text-violet-700 dark:text-violet-300 border border-violet-500/20 font-bold tracking-wider uppercase">Manual</span>
              )}
              {hasProvenance && src !== "agent_simulated" && src !== "manual" && src !== "manual_import" && (
                <span title="Surfaced by the AI sourcing agent" className="inline-flex items-center gap-1 text-[9px] px-2 py-0.5 rounded-full bg-primary/10 text-primary border border-primary/20 font-bold tracking-wider uppercase">
        ...
        {verification && (() => {
          const verdict = verification.verdict || (verificationStatus === "verified" ? "clear" : verificationStatus === "flagged" ? "flag" : "review");
          const cfg = verdict === "clear"
            ? { label: "Verified", cls: "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border-emerald-500/25", Icon: ShieldCheck }
            : verdict === "flag"
            ? { label: "Flagged", cls: "bg-rose-500/10 text-rose-700 dark:text-rose-300 border-rose-500/25", Icon: ShieldAlert }
            : { label: "Needs review", cls: "bg-amber-500/10 text-amber-700 dark:text-amber-300 border-amber-500/25", Icon: ShieldQuestion };
          const score = typeof verification.overallScore === "number" ? verification.overallScore : null;
          const flagCount = Array.isArray(verification.riskFlags) ? verification.riskFlags.length : 0;
          return (
            <div className={cn("flex items-center gap-1.5 text-[10px] font-semibold px-2 py-1 rounded-md border", cfg.cls)}>
              <cfg.Icon className="w-3 h-3 shrink-0" />
              <span className="truncate">{cfg.label}</span>
              {score != null && <span className="opacity-70 font-medium">· {score}/100</span>}
              {flagCount > 0 && <span className="opacity-70 font-medium">· {pluralize(flagCount, "flag")}</span>}
            </div>
          );
        })()}
        ...
        {!verification && !verificationRunning && (verificationStatus === "pending" || row.stage === "verification") && (
          <div className="flex items-center gap-1.5 text-[10px] font-semibold px-2 py-1 rounded-md border bg-amber-500/10 text-amber-700 dark:text-amber-300 border-amber-500/25">
            ...
            <span className="truncate">Verification pending — won't auto-advance to outreach</span>
          </div>
        )}
          ...
          emailEditOpen && c?.id ? (
            <div onClick={(e) => e.stopPropagation()} className="flex items-center gap-1.5">
              <Input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") { e.preventDefault(); const v = emailValue.trim(); if (v) addEmailMutation.mutate(v); }
              ...
              <ActionBtn
                ...
                onClick={(e: React.MouseEvent) => { e.stopPropagation(); const v = emailValue.trim(); if (v) addEmailMutation.mutate(v); }}
              >
                Save
              </ActionBtn>
              <button
                type="button"
                onClick={(e) => { e.stopPropagation(); setEmailEditOpen(false); }}
            ...
            <button
              ...
              title={c?.id ? "Add a contact email so this candidate can be messaged" : "No contact email on file — can't be messaged until one is added"}
            >
              <AlertTriangle className="w-3 h-3 shrink-0" />
              <span className="shrink-0 whitespace-nowrap">No email</span>
              {c?.id && <span className="ml-auto shrink-0 whitespace-nowrap underline underline-offset-2 decoration-rose-500/40">Add email</span>}
            </button>
          )
        )}
        ...
        {((c?.skills?.length > 0) || (row.nba && NBA_LABELS[row.nba])) && (
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {(c?.skills?.length || 0) > 3 && (
              <span className="text-[11px] text-muted-foreground/70 font-medium px-1">+{c.skills.length - 3}</span>
            )}
          </div>
        )}
          ...
          <span className="text-xs text-muted-foreground truncate">
            {(() => {
              ...
              const verb = row.sourcedId
                ? "Sourced"
                : ["applied", "portal", "referral", "self", "self_apply"].includes((c?.source ?? "").toLowerCase())
                  ? "Applied"
                  : "Added";
          ...
          <div className="flex items-center gap-1 shrink-0">
            {(() => {
              ...
              if (row.missingEmail && c?.id && s === "verification")
                return <PrimaryLink label="Add email" icon={Mail} onClick={() => { setEmailValue(""); setEmailEditOpen(true); }} />;
              if (s === "screening" && row.sourcedId)
                return <PrimaryLink label="Verify" icon={ArrowRight} loading={verifyMutation.isPending} disabled={isPending} onClick={() => verifyMutation.mutate()} />;
              if (s === "verification" && row.sourcedId)
                return <PrimaryLink label="Outreach" icon={ArrowRight} loading={outreachMutation.isPending} disabled={isPending || row.missingEmail} title={row.missingEmail ? "Add a contact email before running outreach" : undefined} onClick={() => outreachMutation.mutate()} />;
              if (s === "shortlisted" && draft)
                return <PrimaryLink label="Review draft" icon={Mail} onClick={() => setDraftOpen(true)} />;
              if (s === "shortlisted" && row.sourcedId)
                return <PrimaryLink label="Interview" icon={Video} loading={acceptMutation.isPending} disabled={isPending} onClick={() => acceptMutation.mutate()} />;
              if ((s === "interview" || s === "interview_scheduled") && (row.sourcedId || row.applicationId || c?.id))
                return row.candidate?.interviewSessionId
                  ? <PrimaryLink label="Open interview" icon={ExternalLink} onClick={() => setLocation(`/interviews/${row.candidate?.interviewSessionId}/room`)} />
                  : <PrimaryLink label="Copy link" icon={Copy} loading={generateLinkMutation.isPending} onClick={() => copyOrGenerateLink()} />;
              if (s === "interview_completed")
                ...
                return <PrimaryLink label="Send to HM" icon={ArrowRight} loading={sendToHmMutation.isPending} disabled={isPending} onClick={() => {
                  sendToHmMutation.mutate();
                  if (hmCandidateId) setLocation(`/candidates/${hmCandidateId}?hmShare=1`);
                }} />;
              if ((s === "hm_review" || s === "offer" || s === "offer_recommended") && row.applicationId)
                return <PrimaryLink label="Extend offer" icon={DollarSign} loading={extendOfferMutation.isPending} disabled={isPending} onClick={() => extendOfferMutation.mutate()} />;
              if (s === "offer_extended" && row.applicationId)
                return <PrimaryLink label="Mark accepted" icon={ThumbsUp} tone="success" loading={acceptOfferMutation.isPending} disabled={isPending} onClick={() => acceptOfferMutation.mutate()} />;
              if (s === "offer_accepted" && row.applicationId)
                return <PrimaryLink label="Mark hired" icon={UserCheck} tone="success" loading={hireMutation.isPending} disabled={isPending} onClick={() => hireMutation.mutate()} />;
              if (s === "hired" && row.applicationId)
                return <PrimaryLink label="Mark started" icon={Play} tone="success" loading={startMutation.isPending} disabled={isPending} onClick={() => startMutation.mutate()} />;
              if (!row.sourcedId && !isTerminal && !["offer_extended", "offer_accepted"].includes(s))
                return <PrimaryLink label={s === "sourced" ? "Add" : "Advance"} icon={ArrowRight} loading={isPending} disabled={isPending || (row.missingEmail && s === "verification")} title={row.missingEmail && s === "verification" ? "Add a contact email before moving to Shortlisted" : undefined} onClick={() => advanceMutation.mutate()} />;
              ...
              if (restoreToBoard)
                return (
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
                      <button type="button" disabled={moveWithoutAutomationMutation.isPending} className="inline-flex items-center gap-1 text-xs font-semibold text-primary hover:underline disabled:opacity-50">
...
function RejectCandidateDialog({ row, jobId, open, onClose }: { row: any; jobId: string; open: boolean; onClose: () => void }) {
  const { toast } = useToast();
  const qc = useQueryClient();
  const c = row.candidate || {};
  const candidateName = `${c.firstName ?? ""} ${c.lastName ?? ""}`.trim() || c.email || "this candidate";
  ...
  const preOutreach = !!row.sourcedId && ["sourced", "screening", "verification"].includes(row.stage);
      ...
      <DialogContent className="max-w-md" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4 py-2">
          <div className="text-xs text-muted-foreground">
            {preOutreach ? (
              <>This candidate hasn't been contacted yet, so no email will be sent. The reason and notes you provide here are saved internally for your records.</>
            ) : (
              <>The candidate will receive a sophisticated, professional email letting them know they were not selected — without using blunt language. The reason and notes you provide here are saved internally for your records and are <span className="font-semibold">not</span> shared with the candidate.</>
            )}
          </div>
        ...
        <DialogFooter>
          ...
          >
            {mutation.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin mr-1.5" /> : <XCircle className="w-3.5 h-3.5 mr-1.5" />}
...
function QuickReplyDialog({ row, jobId, open, onClose }: { row: any; jobId: string; open: boolean; onClose: () => void }) {
  const { toast } = useToast();
  const qc = useQueryClient();
  const c = row.candidate || {};
  ...
  const logMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      toast({
        title:
          data.sentiment === "positive" ? "Reply logged — moved to Scheduled" :
          data.sentiment === "negative" ? "Reply logged — candidate declined" :
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <p className="text-xs text-muted-foreground">
            Paste what the candidate emailed back. We'll classify it and advance the stage automatically.
          </p>
        </DialogHeader>
        ...
        <div className="space-y-3">
          ...
          <p className="text-[11px] text-muted-foreground">
            Tip — for production, candidates' email replies can flow in automatically via the inbound webhook at <span className="font-mono">/api/webhooks/inbound-email</span>.
          </p>
        </div>
        ...
        <DialogFooter className="flex-wrap gap-2 sm:justify-between">
          ...
          <div className="flex gap-2">
            ...
            >
              {logMutation.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <ThumbsUp className="w-3.5 h-3.5" />}
...
function RejectionDetailsSection({
  candidateId, sourcedId, applicationId, jobId, enabled,
}: {
    ...
    return (
      <div className={wrap}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-rose-300 uppercase tracking-wide">Rejected</span>
        </div>
  ...
  const actor = r.actorName || r.actorEmail || (r.rejectedByRole === "system" ? "System / automation" : "Unknown");
  ...
  return (
    <div className={wrap}>
      ...
      {r.reason ? (
        <div>
          <div className="text-[10px] font-semibold text-rose-300/80 uppercase tracking-wide mb-0.5">Reason</div>
      ...
      {r.notes && (
        <div>
          <div className="text-[10px] font-semibold text-rose-300/80 uppercase tracking-wide mb-0.5">Notes</div>
      ...
      <div className="flex items-center gap-2 flex-wrap text-[11px] text-muted-foreground pt-1 border-t border-rose-500/15">
        <span className="flex items-center gap-1">
          ...
          {r.rejectedByRole && (
            <span className="text-muted-foreground/60">· {r.rejectedByRole.replace(/_/g, " ")}</span>
          )}
        </span>
        <span className="ml-auto flex items-center gap-1">
          ...
          {r.emailSent ? (
            ...
            <span className="text-amber-400" title={r.emailError}>Email failed</span>
          ) : (
...
function CandidateDetailDialog({ row, jobId, open, onClose }: { row: any; jobId: string; open: boolean; onClose: () => void }) {
  const { toast } = useToast();
  const qc = useQueryClient();
  const [, setLocation] = useLocation();
  const c = row.candidate || {};
  ...
  const verification = row.verificationResult || c.verificationResult || row.candidate?.verificationResult;
  ...
  const auditIds = [row.normalizedCandidateId, row.candidate?.id, row.sourcedId].filter(Boolean) as string[];
  ...
  const logReplyMutation = useMutation({
    mutationFn: (sentiment: "positive" | "negative" | "do_not_contact") =>
      apiFetch<any>(`/jobs/${effectiveJobId || ""}/pipeline/card-action`, {
        method: "POST",
        body: JSON.stringify({ action: "log_reply", sourcedId: row.sourcedId, sentiment, replyBody: replyText }),
      }),
    onSuccess: (data: any) => {
      toast({
        title: data.sentiment === "positive" ? "Reply logged — moved to Scheduled" :
               data.sentiment === "negative" ? "Reply logged — candidate declined" :
               "Marked as Do Not Contact",
      });
      ...
      qc.invalidateQueries({ queryKey: ["/api/candidates"] });
      qc.invalidateQueries({ queryKey: ["dnc-list"] });
      if (effectiveJobId) qc.invalidateQueries({ queryKey: ["intelligence", "job", effectiveJobId] });
      onClose();
    },
    onError: (err: any) =>
      toast({ title: "Failed to log reply", description: err?.message, variant: "destructive" }),
  ...
  const verdictBadge = (verdict?: string) => {
    ...
    return <span className={cn("text-[10px] px-2 py-0.5 rounded border font-semibold uppercase", map[verdict] || "text-muted-foreground border-border")}>{verdict}</span>;
  };
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <DialogTitle className="flex items-center gap-2">
            {profileCandidateId ? (
              <button
                ...
                onClick={() => { onClose(); setLocation(`/candidates/${profileCandidateId}`); }}
              >
            ...
            {(row.score ?? c.resumeScreenScore) != null && <ScorePill score={row.score ?? c.resumeScreenScore} />}
          </DialogTitle>
          <p className="text-xs text-muted-foreground">
            {c.currentTitle}{c.currentCompany ? ` · ${c.currentCompany}` : ""}{c.location ? ` · ${c.location}` : ""}
          </p>
        ...
        <div className="space-y-5 text-sm">
          ...
          {(c.skills?.length ?? 0) > 0 && (
            <div>
              <div className="text-xs font-semibold text-muted-foreground uppercase mb-1.5">Skills</div>
              <div className="flex flex-wrap gap-1">
                {c.skills.map((s: string) => <Badge key={s} variant="secondary" className="text-[10px]">{s}</Badge>)}
              </div>
            </div>
          )}
          ...
          {screening ? (
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <div className="text-xs font-semibold text-muted-foreground uppercase">AI Screening</div>
                {verdictBadge(screening.recommendation)}
                {screening.score != null && <span className="text-[10px] text-muted-foreground">score {screening.score}/100</span>}
                {screening.confidence && <span className="text-[10px] text-muted-foreground">· confidence {screening.confidence}</span>}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {screening.strengthAreas?.length > 0 && (
                  <div>
                    <div className="text-[10px] font-semibold text-emerald-400 uppercase mb-1">Strengths</div>
                    <ul className="text-xs space-y-0.5">{screening.strengthAreas.map((s: string, i: number) => <li key={i} className="flex gap-1.5"><CheckCircle2 className="w-3 h-3 text-emerald-400 shrink-0 mt-0.5" /><span>{s}</span></li>)}</ul>
                  </div>
                )}
                {screening.gapAreas?.length > 0 && (
                  <div>
                    <div className="text-[10px] font-semibold text-amber-400 uppercase mb-1">Gaps / Unknowns</div>
                    <ul className="text-xs space-y-0.5">{screening.gapAreas.map((s: string, i: number) => <li key={i} className="flex gap-1.5"><AlertTriangle className="w-3 h-3 text-amber-400 shrink-0 mt-0.5" /><span>{s}</span></li>)}</ul>
                  </div>
                )}
              </div>
          ...
          {verification ? (
            <div className="space-y-2 border-t border-border/50 pt-4">
              <div className="flex items-center gap-2">
                <div className="text-xs font-semibold text-muted-foreground uppercase">Identity Verification</div>
                {verdictBadge(verification.verdict)}
                {verificationStatus && <span className="text-[10px] text-muted-foreground">· status: {verificationStatus}</span>}
                {verification.overallScore != null && <span className="text-[10px] text-muted-foreground">· score {verification.overallScore}/100</span>}
              </div>
              {verification.notes && (
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                {verification.linkedinMatch && <div><span className="text-muted-foreground">LinkedIn: </span>{verification.linkedinMatch}</div>}
                {verification.emailValidity && <div><span className="text-muted-foreground">Email: </span>{verification.emailValidity}</div>}
                {verification.resumeConsistency && <div><span className="text-muted-foreground">Resume: </span>{verification.resumeConsistency}</div>}
                {verification.profileCompleteness != null && <div><span className="text-muted-foreground">Profile completeness: </span>{verification.profileCompleteness}%</div>}
              </div>
              {verification.checksPerformed?.length > 0 && (
                <div className="text-xs">
                  <div className="text-[10px] font-semibold text-muted-foreground uppercase mb-1">Checks performed</div>
                  <ul className="space-y-0.5">{verification.checksPerformed.map((s: string, i: number) => <li key={i} className="flex gap-1.5"><CheckCircle2 className="w-3 h-3 text-cyan-400 shrink-0 mt-0.5" /><span>{s}</span></li>)}</ul>
                </div>
              )}
              {verification.riskFlags?.length > 0 && (
                <div className="text-xs">
                  <div className="text-[10px] font-semibold text-rose-400 uppercase mb-1">Risk flags</div>
                  <ul className="space-y-0.5">{verification.riskFlags.map((s: string, i: number) => <li key={i} className="flex gap-1.5"><XCircle className="w-3 h-3 text-rose-400 shrink-0 mt-0.5" /><span>{s}</span></li>)}</ul>
                </div>
              )}
            </div>
          ) : (
          ...
          {(row.stage === "shortlisted" || row.stage === "interview_scheduled" || replyStatus) && (
            <div className="space-y-2 border-t border-border/50 pt-4">
              <div className="flex items-center gap-2">
                <div className="text-xs font-semibold text-muted-foreground uppercase">Candidate Response</div>
                {replyStatus && (
                  <span className={cn(
                    "text-[10px] px-2 py-0.5 rounded border font-semibold uppercase",
                    replyStatus === "positive" ? "text-emerald-400 bg-emerald-500/10 border-emerald-500/30" :
                    replyStatus === "negative" ? "text-rose-400 bg-rose-500/10 border-rose-500/30" :
              ...
              {!replyStatus && (
                <>
                  <p className="text-xs text-muted-foreground">
                    Got a reply from this candidate? Log it here. The Inbox at <span className="font-mono">/outreach/inbox</span> shows all replies grouped by job.
                  </p>
                  <textarea
          ...
          {commEvents.length > 0 && (
            <div className="space-y-2 border-t border-border/50 pt-4">
              <div className="flex items-center gap-2">
                <div className="text-xs font-semibold text-muted-foreground uppercase">Communication Log</div>
                <span className="text-[10px] text-muted-foreground">{pluralize(commEvents.length, "event")}</span>
              </div>
              <div className="max-h-64 overflow-y-auto space-y-2 pr-1">
                {commEvents.map((ev: any) => {
                  const ts = ev.sentAt || ev.createdAt;
                  const tone =
                    ev.status === "failed" ? "border-rose-500/30 bg-rose-500/5" :
                    ev.status === "opened" ? "border-emerald-500/30 bg-emerald-500/5" :
                    ev.type === "re_engagement" ? "border-amber-500/30 bg-amber-500/5" :
                    "border-border bg-muted/30";
                  return (
                    <div key={ev.id} className={cn("text-xs p-2 rounded border", tone)}>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[10px] font-semibold uppercase text-muted-foreground">{ev.type.replace(/_/g, " ")}</span>
                        ...
                        <span className="text-[10px] text-muted-foreground ml-auto">{ts ? new Date(ts).toLocaleString() : ""}</span>
                      </div>
...
/* ── Add Candidate Modal ──────────────────────────────────────────────── */
function AddCandidateModal({ jobId, open, onClose }: { jobId: string; open: boolean; onClose: () => void }) {
  const { toast } = useToast();
  const qc = useQueryClient();
  const cvRef = useRef<HTMLInputElement>(null);
  const [form, setForm] = useState({ firstName: "", lastName: "", email: "", currentTitle: "", currentCompany: "", location: "", skills: "", linkedinUrl: "", notes: "" });
  ...
  const [emailMatch, setEmailMatch] = useState<{ existing: any; proposedChanges: { field: string; label: string; from: any; to: any }[] } | null>(null);

  const resetAndClose = () => {
    setForm({ firstName: "", lastName: "", email: "", currentTitle: "", currentCompany: "", location: "", skills: "", linkedinUrl: "", notes: "" });
    setCvFile(null);
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (res.status === 409 && data?.reason === "email_match" && data?.existing) {
        return { emailMatch: { existing: data.existing, proposedChanges: Array.isArray(data.proposedChanges) ? data.proposedChanges : [] } };
      }
      if (!res.ok) throw new Error(data?.error || `API ${res.status}`);
      return { ok: true };
    },
    onSuccess: (result: any) => {
      if (result?.emailMatch) { setEmailMatch(result.emailMatch); return; }
  ...
  return (
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div
          className={cn(
            ...
            cvFile ? "border-emerald-500/50 bg-emerald-500/5" : "border-border/50 hover:border-primary/40 hover:bg-primary/5"
          )}
        ...
        >
          ...
          {parsing ? (
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm font-medium">Upload CV / Resume <span className="text-muted-foreground font-normal">(optional)</span></p>
              <p className="text-[11px] text-muted-foreground">PDF, DOCX, or TXT — AI will pre-fill the fields below</p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            <Input className="h-8 text-sm" placeholder="John" value={form.firstName} onChange={set("firstName")} />
          </div>
          <div className="space-y-1">
            <Label className="text-xs">Last Name *</Label>
            <Input className="h-8 text-sm" placeholder="Doe" value={form.lastName} onChange={set("lastName")} />
          </div>
          <div className="col-span-2 space-y-1">
            <Label className="text-xs">Email</Label>
            <Input className="h-8 text-sm" type="email" placeholder="john@example.com" value={form.email} onChange={set("email")} />
          </div>
          <div className="space-y-1">
            <Label className="text-xs">Current Title</Label>
            <Input className="h-8 text-sm" placeholder="Senior Developer" value={form.currentTitle} onChange={set("currentTitle")} />
          </div>
          <div className="space-y-1">
            <Label className="text-xs">Company</Label>
            <Input className="h-8 text-sm" placeholder="Acme Corp" value={form.currentCompany} onChange={set("currentCompany")} />
          </div>
          <div className="space-y-1">
            <Label className="text-xs">Location</Label>
            <Input className="h-8 text-sm" placeholder="Buenos Aires, AR" value={form.location} onChange={set("location")} />
          </div>
          <div className="space-y-1">
            <Label className="text-xs">LinkedIn URL</Label>
            <Input className="h-8 text-sm" placeholder="linkedin.com/in/..." value={form.linkedinUrl} onChange={set("linkedinUrl")} />
          </div>
          <div className="col-span-2 space-y-1">
            <Label className="text-xs">Skills <span className="text-muted-foreground">(comma-separated)</span></Label>
            <Input className="h-8 text-sm" placeholder="Java, Spring Boot, PostgreSQL" value={form.skills} onChange={set("skills")} />
          </div>
          <div className="col-span-2 space-y-1">
            <Label className="text-xs">Notes</Label>
            <Input className="h-8 text-sm" placeholder="Referred by..." value={form.notes} onChange={set("notes")} />
          </div>
        </div>
        ...
        <DialogFooter className="mt-2">
          ...
          <Button
            ...
            onClick={() => mutation.mutate()}
          >
            {mutation.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin mr-1.5" /> : <UserPlus className="w-3.5 h-3.5 mr-1.5" />}
            Add to Pipeline
          </Button>
        </DialogFooter>
      </DialogContent>
    ...
    <Dialog open={!!emailMatch} onOpenChange={(o) => { if (!o) setEmailMatch(null); }}>
        ...
        {emailMatch && (
          <div className="space-y-3 text-sm">
            ...
            <div className="rounded-md border border-border/40 bg-card/40 p-3">
              <div className="font-medium">{emailMatch.existing.firstName} {emailMatch.existing.lastName}</div>
              {emailMatch.existing.email && <div className="text-xs text-muted-foreground">{emailMatch.existing.email}</div>}
              {emailMatch.existing.currentTitle && <div className="text-xs text-muted-foreground">{emailMatch.existing.currentTitle}</div>}
            </div>
            {emailMatch.proposedChanges.length > 0 ? (
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground">The info you entered is newer for these fields. Updating merges them into the existing record (nothing is blanked out):</p>
                <div className="rounded-md border border-border/40 divide-y divide-border/40">
                  {emailMatch.proposedChanges.map((c) => (
                    <div key={c.field} className="px-3 py-2 text-xs">
                      ...
                      <div className="text-muted-foreground line-through">{c.from ? String(c.from) : "—"}</div>
              ...
              <p className="text-xs text-muted-foreground">Your entry doesn't add any newer info, but we can still add this candidate to the role.</p>
            )}
          </div>
        )}
        <DialogFooter>
          <Button type="button" variant="outline" size="sm" onClick={() => setEmailMatch(null)} disabled={mergeMutation.isPending} data-testid="button-email-match-cancel">Cancel</Button>
          <Button type="button" size="sm" onClick={() => mergeMutation.mutate()} disabled={mergeMutation.isPending} data-testid="button-email-match-merge">
            {mergeMutation.isPending ? <><Loader2 className="w-3.5 h-3.5 animate-spin mr-1.5" />Updating…</> : (emailMatch && emailMatch.proposedChanges.length > 0 ? "Update existing" : "Add to role")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
/* ── CSV helpers ──────────────────────────────────────────────────────── */
const CSV_HEADERS = ["firstName","lastName","email","phone","location","currentTitle","currentCompany","linkedinUrl","githubUrl","skills"];
...
/* ── Bulk Upload Modal ────────────────────────────────────────────────── */
function BulkUploadModal({ jobId, open, onClose }: { jobId: string; open: boolean; onClose: () => void }) {
  const { toast } = useToast();
  const qc = useQueryClient();
  ...
  const mutation = useMutation({
    mutationFn: () => apiFetch<any>("/candidates/bulk-import", {
      method: "POST",
  ...
  const downloadTemplate = () => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = "lexy_candidates_template.csv"; a.click();
        ...
        {!result ? (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">Upload a CSV file to add multiple candidates to this job's pipeline at once.</p>
              <Button size="sm" variant="ghost" className="gap-1.5 text-xs shrink-0" onClick={downloadTemplate}>
            ...
            >
              ...
              {fileName ? (
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">firstName, lastName, email are required · up to 500 rows</p>
                </div>
              )}
            </div>
            ...
            {rows.length > 0 && (
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground font-medium">Preview (first {Math.min(5, rows.length)} of {rows.length} rows)</p>
                <div className="rounded-lg border border-border/50 overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-border/50 bg-muted/30">
                        {previewCols.map(col => <th key={col} className="px-3 py-2 text-left font-medium text-muted-foreground">{col}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                ...
                {rows.length > 5 && <p className="text-xs text-muted-foreground/60 text-right">+ {rows.length - 5} more rows</p>}
              </div>
            )}
          </div>
        ) : (
            ...
            {result.errors.length > 0 && (
              <div className="rounded-lg border border-rose-500/20 bg-rose-500/5 p-3 space-y-1 max-h-36 overflow-y-auto">
                ...
                {result.errors.map((e: any, i: number) => (
                  <p key={i} className="text-xs text-muted-foreground">Row {e.row} ({e.email}): {e.reason}</p>
                ))}
              </div>
            )}
          </div>
...
/* ── CV Upload Modal ──────────────────────────────────────────────────── */
function CvUploadModal({ jobId, open, onClose }: { jobId: string; open: boolean; onClose: () => void }) {
  const { toast } = useToast();
  const qc = useQueryClient();
  const fileRef = useRef<HTMLInputElement>(null);
  ...
  const reset = () => { setFiles([]); setResults(null); if (fileRef.current) fileRef.current.value = ""; };
  const handleClose = () => { reset(); onClose(); };
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && handleClose()}>
      <DialogContent className="max-w-2xl">
        ...
        {!results ? (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Drop PDF, DOCX, TXT resume files, or a CSV roster. AI extracts name, email, title and skills from resumes; CSV rows are imported as-is.
            </p>

            <div
              className={cn(
                "border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-colors",
                dragging ? "border-primary bg-primary/10" : "border-border/50 hover:border-primary/40 hover:bg-primary/5"
              )}
              onClick={() => fileRef.current?.click()}
              ...
              onDrop={handleDrop}
            >
              <input ref={fileRef} type="file" accept=".pdf,.docx,.txt,.csv" multiple className="hidden" onChange={e => addFiles(e.target.files)} />
              <Upload className="w-8 h-8 text-muted-foreground/50 mx-auto mb-2" />
              <p className="text-sm font-medium">Click or drag &amp; drop CV or CSV files here</p>
              <p className="text-xs text-muted-foreground mt-1">PDF, DOCX, TXT, CSV · up to 10 MB each · up to 20 files</p>
            </div>

            {files.length > 0 && (
              <div className="space-y-1.5">
                {files.map(f => (
                  <div key={f.name} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted/30 border border-border/40">
                    <FileText className="w-3.5 h-3.5 text-muted-foreground shrink-0" />
                    <span className="text-xs flex-1 truncate">{f.name}</span>
                    <span className="text-[10px] text-muted-foreground shrink-0">{(f.size / 1024).toFixed(0)} KB</span>
                    <button onClick={() => removeFile(f.name)} aria-label={`Remove file ${f.name}`} className="text-muted-foreground/50 hover:text-rose-400 transition-colors">
                  ...
                  {successResults.map((r: any, i: number) => (
                    <div key={i} className="px-3 py-2.5 flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground truncate">{r.currentTitle ?? "—"} {r.currentCompany ? `at ${r.currentCompany}` : ""}</p>
                        {r.email && <p className="text-[10px] text-muted-foreground/60">{r.email}</p>}
                        {r.skills?.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1">
                            {r.skills.slice(0, 5).map((s: string, si: number) => (
                              <span key={si} className="text-[9px] px-1.5 py-0.5 rounded bg-primary/10 text-primary border border-primary/20">{s}</span>
                            ))}
                            {r.skills.length > 5 && <span className="text-[9px] text-muted-foreground">+{r.skills.length - 5}</span>}
                          </div>
                        )}
                        {r.summary && <p className="text-[10px] text-muted-foreground/60 mt-1 line-clamp-2">{r.summary}</p>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {errorResults.length > 0 && (
              <div className="rounded-lg border border-rose-500/20 bg-rose-500/5 p-3 space-y-1">
                ...
                {errorResults.map((r: any, i: number) => (
                  <p key={i} className="text-xs text-muted-foreground"><span className="font-medium">{r.fileName}</span>: {r.error}</p>
                ))}
              </div>
            )}
          </div>
        ...
        <DialogFooter>
          <Button variant="ghost" size="sm" onClick={handleClose}>{results ? "Close" : "Cancel"}</Button>
          {!results && (
            <Button size="sm" disabled={files.length === 0 || parsing} onClick={parse}>
              {parsing
                ? <><Loader2 className="w-3.5 h-3.5 animate-spin mr-1.5" /> Parsing {pluralize(files.length, "file")} with AI…</>
                : <><FileText className="w-3.5 h-3.5 mr-1.5" /> Parse &amp; Import {pluralize(files.length, "CV")}</>
              }
            </Button>
          )}
          {results && (
            <Button size="sm" variant="outline" onClick={reset}>Upload More</Button>
          )}
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function InterviewSetupModal({ jobId, open, onClose }: { jobId: string; open: boolean; onClose: () => void }) {
  const { toast } = useToast();
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg">
        ...
        <p className="text-xs text-muted-foreground -mt-1">
          Add a few specific questions or a general direction for this job's interviews.
          Applies to every candidate who reaches the interview stage — both auto-scheduled
          and manually generated links.
        </p>
          ...
          <div className="space-y-4 py-1">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-2">
                ...
                />
                <Button size="sm" variant="outline" className="h-8 px-2 flex-shrink-0" onClick={addQuestion}>
...
function ClosedDrawer({ jobId, open, onClose, winRows, lossRows, onMove }: {
  jobId: string; open: boolean; onClose: () => void;
  winRows: any[]; lossRows: any[]; onMove: () => void;
}) {
  return (
    <Sheet open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <SheetContent side="right" className="w-full sm:max-w-md overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>
            Wins and closed candidates leave the active board. Use a card's <span className="font-medium text-foreground">Restore to board</span> action to move anyone back into a working stage — no emails or agents are triggered.
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="mt-6 space-y-8">
          <section>
            ...
            {winRows.length === 0 ? (
              <p className="text-xs text-muted-foreground/70 border border-dashed border-border/50 rounded-xl px-3 py-6 text-center">No hires yet — closed wins will appear here.</p>
            ) : (
              <div className="space-y-3">
                {winRows.map((row, i) => (
                  <TerminalRow key={row.applicationId || row.sourcedId || i} row={row} jobId={jobId} onMove={onMove} />
                ))}
              </div>
            )}
          </section>
          ...
          <section>
            ...
            {lossRows.length === 0 ? (
              <p className="text-xs text-muted-foreground/70 border border-dashed border-border/50 rounded-xl px-3 py-6 text-center">No rejected or declined candidates.</p>
            ) : (
              <div className="space-y-3">
                {lossRows.map((row, i) => (
                  <TerminalRow key={row.applicationId || row.sourcedId || i} row={row} jobId={jobId} onMove={onMove} />
...
export function PipelinePanel({ jobId }: { jobId: string }) {
  ...
  const [expandAll, setExpandAll] = useState<boolean>(() => {
    try { return localStorage.getItem("lexy.pipeline.expandEmpty") === "1"; } catch { return false; }
  });
  const [tempExpanded, setTempExpanded] = useState<Set<string>>(() => new Set());
  useEffect(() => {
    try { localStorage.setItem("lexy.pipeline.expandEmpty", expandAll ? "1" : "0"); } catch { /* ignore */ }
  }, [expandAll]);
  ...
  useEffect(() => {
    ...
    const onDown = (e: MouseEvent) => {
      ...
      setTempExpanded(prev => {
        ...
        prev.forEach(k => { if ((stages[k]?.length ?? 0) > 0) next.add(k); });
  ...
  const winRows: any[] = STAGE_COLS.filter(c => c.terminal === "win").flatMap(c => stages[c.key] ?? []);
  const lossRows: any[] = STAGE_COLS.filter(c => c.terminal === "loss").flatMap(c => stages[c.key] ?? []);
  ...
  const renderColumn = (col: { key: string; label: string }, phase: Phase | null) => {
      ...
      return (
        <button
          ...
          className={cn(
            ...
            ps.stripBg, ps.stripBorder,
          )}
        >
          <span className={cn("text-[10px] font-bold leading-none", ps.stripCount)}>0</span>
          <span
            className={cn("mt-1 text-[11px] font-semibold tracking-wide whitespace-nowrap", ps.stripText)}
    ...
    return (
      <div
        ...
        <div className={cn("flex items-center justify-between px-3 py-2 rounded-lg border text-xs font-bold", ps.header)}>
          <span>{col.label}</span>
          {count > 0
            ? <span className={cn("inline-flex items-center justify-center min-w-[1.25rem] px-1.5 py-0.5 rounded-full text-[10px] font-bold leading-none", ps.pill)}>{count}</span>
        ...
        <div className="space-y-2">
          {cards.map((row: any, i: number) => (
            <CandidateCard
              ...
              row={row}
              jobId={jobId}
              draft={(row.candidate?.id && draftsByCandidate[row.candidate.id]) || (row.sourcedId && draftsByCandidate[row.sourcedId]) || undefined}
          ...
          {count === 0 && (
            <div className="border-2 border-dashed border-slate-300/80 rounded-xl h-24 flex items-center justify-center bg-white/60 dark:border dark:border-dashed dark:border-border/40 dark:bg-transparent">
              <p className="text-[10px] text-muted-foreground/60 dark:text-muted-foreground/50">Empty</p>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-4 flex-wrap text-sm text-muted-foreground">
          {isLoading
            ? <span className="flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Loading pipeline…</span>
            : <>
                <span className="flex items-center gap-1.5"><Users className="w-4 h-4" /> <strong className="text-foreground">{total}</strong> candidates total</span>
                {STAGE_COLS.filter(c => (stages[c.key]?.length ?? 0) > 0).map(col => {
                  const ph = STAGE_PHASE[col.key];
                  return (
                    <span key={col.key} className={cn("flex items-center gap-1", ph ? PHASE_STYLE[ph].text : "text-muted-foreground")}>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            title={expandAll ? "Collapse empty stages into slim strips" : "Expand every stage to full width"}
          >
            {expandAll
              ? <><Minimize2 className="w-3.5 h-3.5" /> Collapse empty</>
              : <><Maximize2 className="w-3.5 h-3.5" /> Expand all</>}
          </Button>
          <Button size="sm" variant="outline" className="gap-1.5 shrink-0" onClick={() => setInterviewSetupOpen(true)}>
            <Video className="w-3.5 h-3.5" /> Interview Setup
          </Button>
          <Button size="sm" variant="outline" className="gap-1.5 shrink-0" onClick={() => setCvOpen(true)}>
            <FileText className="w-3.5 h-3.5" /> Upload CVs
          </Button>
          <Button size="sm" variant="outline" className="gap-1.5 shrink-0" onClick={() => setBulkOpen(true)}>
            <Upload className="w-3.5 h-3.5" /> Bulk Upload
          </Button>
          <Button size="sm" variant="outline" className="gap-1.5 shrink-0" onClick={() => setAddOpen(true)}>
      ...
      <div className="flex items-center gap-2 text-[10px] text-muted-foreground/60 flex-wrap">
        <span>Sourced</span><ArrowRight className="w-2.5 h-2.5" />
        <span>Screening</span><ArrowRight className="w-2.5 h-2.5" />
        <span className="text-amber-400/70">Verify</span><ArrowRight className="w-2.5 h-2.5" />
        <button
          type="button"
        ...
        </button><ArrowRight className="w-2.5 h-2.5" />
        <span className="text-violet-400/70">Outreach</span><ArrowRight className="w-2.5 h-2.5" />
        <span className="text-emerald-400/70">Candidate Accepts</span><ArrowRight className="w-2.5 h-2.5" />
        <span className="text-emerald-400/70">Interview</span><ArrowRight className="w-2.5 h-2.5" />
        <span className="text-fuchsia-400/70">HM Review</span><ArrowRight className="w-2.5 h-2.5" />
        <span className="text-yellow-400/70">Offer Extended</span><ArrowRight className="w-2.5 h-2.5" />
        <span className="text-lime-400/70">Offer Accepted</span><ArrowRight className="w-2.5 h-2.5" />
        <span className="text-emerald-400/70">Hired</span><ArrowRight className="w-2.5 h-2.5" />
      ...
      {total === 0 && !isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4 border border-dashed border-border/50 rounded-xl">
          ...
          <div className="text-center space-y-1">
            ...
            <p className="text-sm text-muted-foreground max-w-sm">
              Run the <strong>Sourcing</strong> and <strong>Screening</strong> agents from the Workflow tab, or add a candidate manually.
            </p>
          </div>
        ...
        <div className="kanban-scroll flex gap-8 overflow-x-auto pb-2 -mx-1 px-1">
            ...
            return (
              <div key={phase} className="flex flex-col gap-3">
                <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm">
                  <div className="px-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">{PHASE_STYLE[phase].label}</div>
                  <div className="mt-1 h-px w-full bg-border" />
                </div>
                <div className="flex gap-4">
                  {cols.map(col => renderColumn(col, phase))}
                </div>
              </div>
            );
          })}
          ...
          <div className="flex flex-col gap-3">
            <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm select-none" aria-hidden>
              <div className="px-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-transparent">·</div>
            ...
            <div className="flex gap-4">
              <button
                ...
                className={cn(
                  "group shrink-0 w-11 self-stretch flex flex-col items-center gap-1 py-2 rounded-2xl border transition-[background-color,border-color,filter] duration-[250ms] ease-[cubic-bezier(0.25,0.1,0.25,1)] motion-reduce:transition-none hover:brightness-110",
                  winRows.length > 0 ? "bg-signal-green/[0.06] border-signal-green/25" : "bg-muted/30 border-border/40",
                )}
              >
                <span className={cn("text-[11px] font-bold leading-none", winRows.length > 0 ? "text-signal-green" : "text-muted-foreground/50")}>{winRows.length}</span>
                <Check className={cn("w-3 h-3", winRows.length > 0 ? "text-signal-green" : "text-muted-foreground/40")} />
                <span
                  className={cn("mt-1 text-[11px] font-semibold tracking-wide whitespace-nowrap", winRows.length > 0 ? "text-signal-green/80" : "text-muted-foreground/60")}
                  style={{ writingMode: "vertical-rl" }}
                >
                  Hired
                </span>
      ...
      <InterviewSetupModal jobId={jobId} open={interviewSetupOpen} onClose={() => setInterviewSetupOpen(false)} />
    </div>
  );
}
