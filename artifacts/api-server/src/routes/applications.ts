...
/* ── Map helper ───────────────────────────────────────────────────────────── */
/**
 * True when the caller's allowed tenant list covers this application — either
 * via the row's own tenant or via the tenant of the requisition it belongs to.
 * Application rows can legitimately live in a different tenant than their job
 * (agency pool candidate → client sub-tenant requisition), and recruiter_admin
 * data scopes don't always include the agency tenant itself, so scoping on the
 * row tenant alone produced false 404s on advance.
 */
async function tenantScopeCoversApplication(
  allowed: string[],
  rowTenantId: string | null,
  jobId: string | null,
): Promise<boolean> {
  if (rowTenantId && allowed.includes(rowTenantId)) return true;
  ...
  const [j] = await db
    ...
    .limit(1);
  return !!j?.tenantId && allowed.includes(j.tenantId);
}
...
router.get("/applications", async (req, res) => {
  ...
  if (candIds.length > 0) {
    const erased = await db
      .select({ id: candidatesTable.id, dataErasedAt: candidatesTable.dataErasedAt })
      .from(candidatesTable)
...
router.post("/applications", validate({ body: CreateApplicationBody }), async (req, res) => {
  ...
  const [cand] = await db
    .select({
      ...
      dataErasedAt: (candidatesTable as any).dataErasedAt,
  ...
  if ((cand as any).dataErasedAt) {
    res.status(410).json({ error: "Candidate data has been erased" });
    return;
  }
  if ((cand as any).doNotContact) {
    res.status(409).json({ error: "Candidate has opted out of contact" });
    return;
  }
  ...
  if (stage === "shortlisted" && !isRealEmail((cand as any).email)) {
    res.status(422).json({
      error:
        "Can't add to Shortlisted: this candidate has no email address on file. Add or enrich an email first.",
  ...
  if (!tenantId || (job.tenantId && cand.tenantId && job.tenantId !== cand.tenantId)) {
    res.status(400).json({ error: "Job and candidate belong to different tenants" });
    return;
  }

  /* Idempotency: one application per (job, candidate). A second "Move to
   * Pipeline" click, a stale tab, or a concurrent request must not create a
   * duplicate pipeline card — return the existing application instead. */
  const [existing] = await db
    ...
    .limit(1);
  if (existing) {
    res.status(200).json(await mapApplication(existing));
    return;
  }
  ...
  if (skipAutomation) {
    logger.info(
      { candidateId, jobId, stage: app.stage },
      "Application created via manual placement → skipping Screening/ICP automation",
    );
    return;
  }

  logger.info({ candidateId, jobId }, "Application created → triggering Screening agent");
  const icpExists = jobId
    ? await db.select().from(icpTable).where(eq(icpTable.jobId, jobId)).limit(1)
    : [];

  orchestrator
    .triggerAgent(
      "screening",
      { candidateId, jobId: jobId ?? undefined, applicationId: app.id },
      "orchestrator",
    )
    .then((run) => {
      if (run.status === "completed" && !icpExists.length && jobId) {
        ...
        orchestrator
          .triggerAgent("icp", { jobId }, "orchestrator")
...
router.get("/applications/:applicationId", async (req, res) => {
  ...
  if (user.role !== "platform_admin") {
    const allowed = await getDataScopeTenantIds(user);
    /* An application row's tenant may differ from its job's tenant (e.g. an
     * agency-pool candidate applied to a client sub-tenant's requisition), so
     * accept EITHER: the caller is scoped to the row's tenant OR to the
     * requisition's tenant. Job-tenant visibility is the meaningful boundary —
     * the recruiter ceiling below still gates plain recruiters per-job. */
    if (!allowed || !(await tenantScopeCoversApplication(allowed, a.tenantId, a.jobId))) {
      res.status(404).json({ error: "Not found" });
      return;
    }
  }
...
router.put(
  ...
  validate({ body: UpdateApplicationBody }),
  async (req, res) => {
    const user = await getCallerUser(req);
    ...
    const [existing] = await db
      .select({ tenantId: applicationsTable.tenantId, jobId: applicationsTable.jobId })
      .from(applicationsTable)
      .where(eq(applicationsTable.id, req.params.applicationId))
      .limit(1);
    if (!existing) {
    ...
    if (user.role !== "platform_admin") {
      const allowed = await getDataScopeTenantIds(user);
      /* Same dual-tenant rule as GET by id: the row's tenant may be the agency
       * while the job lives in a client sub-tenant (or vice versa). */
      if (
        !allowed ||
        !(await tenantScopeCoversApplication(allowed, existing.tenantId, existing.jobId))
      ) {
        res.status(404).json({ error: "Not found" });
        return;
      }
    ...
    if (req.body.stage) {
      ...
      if (transitioningToOutreach && current?.candidateId) {
        ...
        if (!isRealEmail(emailCand?.email)) {
          res.status(422).json({
            error:
              "Can't move to Shortlisted: this candidate has no email address on file. Add or enrich an email first.",
            code: "NO_CONTACT_EMAIL",
            recoverable: true,
          });
          return;
      ...
      if (!skipAutomation && target !== "rejected" && target !== currentStage) {
        ...
        if (target === "hired" && !validHiredFromStages.includes(currentStage)) {
          res
            .status(400)
            .json({ error: "Cannot move to Hired: candidate must be in an Offer stage first." });
          return;
        }
        ...
        if (target === "started" && currentStage !== "hired") {
          res
            .status(400)
            .json({ error: "Cannot move to Started: candidate must be in Hired stage first." });
          return;
        }
      }
    }
    ...
    if (req.body.stage && candidateId) {
      ...
      try {
        ...
        logger.error(
          { err, applicationId: req.params.applicationId },
          "Stage change transaction failed — rolled back (stage/mirror/event/audit)",
        );
    ...
    if (transitioningToRejected && a.candidateId) {
      ...
      const rejectionNotes: string | null =
        req.body.rejectionNotes ?? req.body.rejection_notes ?? null;
      (async () => {
        try {
      ...
      ensureCandidateUser(a.candidateId, a.tenantId)
        ...
        .then(
          (token) =>
            token &&
            logger.info(
              { candidateId: a.candidateId, stage: req.body.stage, token },
              "Auto-invite generated on stage advance",
            ),
        )
    ...
    if (transitioningToScreening && a.candidateId && !skipAutomation) {
      logger.info(
        { applicationId: a.id, candidateId: a.candidateId, jobId: a.jobId },
        "Application advanced to Screening → triggering Screening agent",
      );
      orchestrator
        .triggerAgent(
          "screening",
          { candidateId: a.candidateId, jobId: a.jobId ?? undefined, applicationId: a.id },
          "orchestrator",
        )
        .catch((err) =>
          logger.error(
            { err, candidateId: a.candidateId },
            "Screening agent failed after advance to screening",
          ),
        );
    }
    ...
    if (transitioningToVerification && a.candidateId && !skipAutomation) {
      logger.info(
        { applicationId: a.id, candidateId: a.candidateId, jobId: a.jobId },
        "Application advanced to Verify → triggering Verification agent",
      );
      import("../lib/run-verification.js")
        .then(({ runCandidateVerification }) =>
          runCandidateVerification({ candidateId: a.candidateId, tenantId: a.tenantId }),
        )
        .catch((err) =>
          logger.error(
            { err, candidateId: a.candidateId },
            "Verification agent failed after advance to verification",
          ),
        );
    }
    ...
    if (transitioningToOutreach && a.candidateId && !skipAutomation) {
      logger.info(
        { applicationId: a.id, candidateId: a.candidateId, jobId: a.jobId },
        "Application advanced to Outreach Queued → triggering Outreach agent (manual recruiter override)",
      );
      orchestrator
        .triggerAgent(
          "outreach",
          {
            ...
            passedCandidateIds: [a.candidateId],
            manualOverride: true,
          },
          "orchestrator",
        )
        .catch((err) =>
          logger.error(
            { err, candidateId: a.candidateId },
            "Outreach agent failed after advance to outreach",
          ),
        );
    }
