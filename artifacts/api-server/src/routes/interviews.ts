...
import { interviewPlansTable, interviewSessionsTable, interviewSummariesTable, interviewSchedulesTable, applicationsTable, candidatesTable, jobsTable, userNotificationsTable, sourcedCandidatesTable, trustEventsTable, aiDecisionLogTable, tenantsTable, candidateAiConsentTable } from "@workspace/db";
...
import { generateWithAI, generateJSON, SUPPORTED_LANGUAGES, resolveLangMeta, azureSpeechConfigured } from "../lib/ai";
...
import { recordSttRequest, getSttMetrics, getSttTrends, getSttWindowSnapshot, type SttProvider } from "../lib/stt-metrics";
...
import { checkInterviewCreationAllowed, buildLimitExceededBody, recordCreditEvent } from "../lib/plan-enforcement";
...
const SaveTurnBody = z.object({
  questionText: z.string().optional(),
  answerText: z.string().optional(),
  turnNumber: z.number().optional(),
}).passthrough();

const WorkAuthBody = z.object({
...
const PatchRecordingBody = z.object({
...
const StartInterviewBody = z.object({
  applicationId: z.string().optional(),
...
const ProctorEventBody = z.object({
  type: z.string().min(1),
...
async function requireActiveAiConsent(req: any, res: any, next: any): Promise<void> {
  try {
    ...
    if (!(await hasActiveAiConsent(req.interviewSession?.candidateId))) {
      res.status(412).json({ error: "AI_CONSENT_REQUIRED", consentVersion: CURRENT_AI_CONSENT_VERSION });
      return;
    }
    next();
  } catch (err) {
    ...
    logger.error({ err, interviewId: req.params?.interviewId }, "[interviews] consent re-check failed");
    res.status(500).json({ error: "consent_check_failed" });
  }
}

router.post("/interviews/:interviewId/transcribe", requireInterviewSessionCookie, requireActiveAiConsent, admit(), async (req, res) => {
  ...
  if (typeof rawPrompt === "string" && rawPrompt) {
    ...
    try { prompt = decodeURIComponent(rawPrompt).slice(0, 2000); }
...
router.post("/interviews/tts", validate({ body: TtsBody }), admit(), async (req, res) => {
  ...
  try {
    ...
    if (langMeta.ttsProvider === "elevenlabs" && process.env.ELEVENLABS_API_KEY) {
      const voiceId = process.env.ELEVENLABS_TTS_VOICE_ID || "EXAVITQu4vr4xnSDxMaL"; /* Sarah — warm female */
      const elAc = new AbortController();
      const elTo = setTimeout(() => elAc.abort(), 9000);
      try {
        const elRes = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${voiceId}?output_format=mp3_44100_96`, {
          method: "POST",
        ...
        logger.warn({ status: elRes.status, language }, "[TTS] ElevenLabs failed — falling back to Azure/OpenAI");
      } catch (err: any) {
        logger.warn({ err: err?.message, language }, "[TTS] ElevenLabs errored/timed out — falling back to Azure/OpenAI");
      } finally {
        clearTimeout(elTo);
      }
    }
    ...
    if (picked) {
      ...
      logger.warn({ status: ttsRes.status, account: picked.account?.id, aborted: threw }, "[TTS] Azure failed — falling back to OpenAI");
    }
    ...
    if (!ttsApiKey) { res.status(503).json({ error: "TTS not configured" }); return; }
...
router.post("/interviews/:interviewId/converse",
  rateLimit({ windowMs: 60_000, max: 60, keyFn: (req) => req.params.interviewId || req.ip || "anon" }),
  ...
  const [plan] = await db.select().from(interviewPlansTable)
    .where(eq(interviewPlansTable.id, session.planId)).limit(1);
  ...
  const [jobRows, candidateRows] = await Promise.all([
    ...
    (session.candidateId && session.candidateId !== "demo")
      ? db.select().from(candidatesTable).where(eq(candidatesTable.id, session.candidateId)).limit(1)
  ...
  const lastCandidateText: string = [...history].reverse()
    .find((h: any) => h.role === "candidate")?.text ?? "";
  const candidateFarewell = !(questionNumber === 1 && history.length === 0)
    && candidateWantsToEnd(lastCandidateText);
  const isClosing = questionNumber > totalQuestions || candidateFarewell;
  const isOpening = questionNumber === 1 && history.length === 0;

  const historyText = history.length > 0
    ? history.map((h: any) => `${h.role === "ai" ? "Lexy" : "Candidate"}: ${h.text}`).join("\n\n")
    : "";

  const topicGuide: Record<string, string> = {
    general:     "background and career journey, motivations, key experiences, working style, challenges overcome, goals",
    technical:   "technical depth, problem-solving approaches, past projects, architectural thinking, tools and technologies",
    behavioral:  "specific past situations, the actions taken, outcomes achieved, collaboration, leadership moments, conflict resolution",
    competency:  "demonstrated skills, knowledge applied in real scenarios, growth areas, how they handle ambiguity",
    cultural:    "alignment with our values, how they handle disagreement and feedback, what kind of team environment they thrive in, examples of living the values they care about, how they've shaped or contributed to a team's culture",
    programming: "how they approach a coding problem from scratch, how they reason about edge cases, trade-offs they consider in design, debugging stories, code-quality habits, testing discipline, language/runtime depth",
  };
  ...
  const recruiterQuestions: string[] = Array.isArray((plan?.culturalConfig as any)?.customQuestions)
    ? ((plan!.culturalConfig as any).customQuestions as any[]).map((q) => (q ?? "").toString().trim()).filter(Boolean)
    : [];
...
router.post("/interviews/:interviewId/save-turn", validate({ body: SaveTurnBody }), requireSameOriginPost, requireInterviewSessionCookie, requireActiveAiConsent, async (req, res) => {
  if (!(await assertSessionJobApproved(res, req.params.interviewId))) return;
...
router.get("/interviews/:interviewId/work-auth-prompt", requireInterviewSessionCookie, async (req, res) => {
  if (!(await assertSessionJobApproved(res, req.params.interviewId))) return;
  const [session] = await db.select().from(interviewSessionsTable)
    .where(eq(interviewSessionsTable.id, req.params.interviewId)).limit(1);
  if (!session) { res.status(404).json({ error: "Not found" }); return; }
  ...
  if (language.startsWith("en")) { res.json({ text: base }); return; }
  try {
...
router.post("/interviews/:interviewId/work-auth", validate({ body: WorkAuthBody }), requireSameOriginPost, requireInterviewSessionCookie, requireActiveAiConsent, async (req, res) => {
  if (!(await assertSessionJobApproved(res, req.params.interviewId))) return;
  ...
  if (!session.candidateId || session.candidateId === "demo" || !answerText.trim()) {
    res.json({ ok: true, persisted: false }); return;
  }
  ...
  try {
    ...
    logger.warn({ interviewId: req.params.interviewId, err: e?.message }, "[job-interview] work-auth extraction failed");
    res.json({ ok: true, persisted: false }); return;
  }
  ...
  if (Object.keys(waUpdate).length === 0) { res.json({ ok: true, persisted: false }); return; }
  ...
  }).from(candidatesTable).where(eq(candidatesTable.id, session.candidateId)).limit(1);
  if (!existingWa) { res.json({ ok: true, persisted: false }); return; }
  ...
  await db.update(candidatesTable).set(waUpdate).where(eq(candidatesTable.id, session.candidateId));
  logger.info({ candidateId: session.candidateId }, "[job-interview] Captured work-authorization (non-scored)");
...
router.get("/interviews/languages", (_req, res) => {
  ...
  const iflytekReady  = !!(process.env.IFLYTEK_APP_ID && process.env.IFLYTEK_API_KEY && process.env.IFLYTEK_API_SECRET);
  const azureReady    = !!(process.env.AZURE_OPENAI_API_KEY && process.env.AZURE_OPENAI_ENDPOINT && process.env.AZURE_OPENAI_DEPLOYMENT);

  res.json(
    Object.entries(SUPPORTED_LANGUAGES).filter(([, meta]) => !meta.hidden).map(([code, meta]) => ({
      code,
      label:          meta.label,
...
router.get("/interviews", async (req: any, res) => {
  ...
  if (caller.role === "platform_admin") {
    sessions = await db.select().from(interviewSessionsTable).orderBy(desc(interviewSessionsTable.createdAt)).limit(MAX_PAGE_SIZE);
  } else {
    const allowedEarly = await getAllowedTenantIds(caller);
    if (!allowedEarly || allowedEarly.length === 0) { res.json([]); return; }
    sessions = await db.select().from(interviewSessionsTable)
  ...
  const enrichTenantIds = caller.role === "platform_admin" ? null : await getAllowedTenantIds(caller);
  const candIds = Array.from(new Set(
    sessions.map(s => s.candidateId).filter(id => id && id !== "demo") as string[]
  ));
  const planIds = Array.from(new Set(sessions.map(s => s.planId).filter(Boolean) as string[]));
  ...
  const candRows = candIds.length
    ? await db.select({ id: candidatesTable.id, firstName: candidatesTable.firstName, lastName: candidatesTable.lastName })
        .from(candidatesTable).where(enrichTenantIds
          ? and(inArray(candidatesTable.id, candIds), inArray(candidatesTable.tenantId, enrichTenantIds))
          : inArray(candidatesTable.id, candIds))
    : [];
  const candMap = new Map(candRows.map(c => [c.id, `${c.firstName ?? ""} ${c.lastName ?? ""}`.trim()]));

  const planRows = planIds.length
    ? await db.select({ id: interviewPlansTable.id, jobId: interviewPlansTable.jobId })
        .from(interviewPlansTable).where(enrichTenantIds
          ? and(inArray(interviewPlansTable.id, planIds), inArray(interviewPlansTable.tenantId, enrichTenantIds))
  ...
  const jobRows = jobIds.length
    ? await db.select({ id: jobsTable.id, title: jobsTable.title })
        .from(jobsTable).where(enrichTenantIds
...
All question text and follow-up prompts must be written in ${langLabel}.`;
  ...
  let questions: any[] = [];
  try {
    const aiResponse = await generateWithAI(prompt, `You are an expert AI interviewer. Generate structured, insightful interview questions. Respond with a valid JSON array only — no markdown fences, no explanation.`, language);
...
async function gateJobForInterview(
  ...
  if (opts?.requireApproved && caller.role !== "platform_admin" && !isJobApprovedForInterview(job.status)) {
    logger.warn({ jobId, status: job.status, userId }, "[interview-gate] blocked — work order not approved");
    res.status(403).json({ error: JOB_NOT_APPROVED_MESSAGE, status: job.status }); return null;
  }
  return { caller, job };
}
...
async function assertSessionJobApproved(res: any, interviewId: string): Promise<boolean> {
  const [session] = await db.select({ planId: interviewSessionsTable.planId })
    .from(interviewSessionsTable).where(eq(interviewSessionsTable.id, interviewId)).limit(1);
  if (!session?.planId) return true;
  const [plan] = await db.select({ jobId: interviewPlansTable.jobId })
    .from(interviewPlansTable).where(eq(interviewPlansTable.id, session.planId)).limit(1);
  if (!plan?.jobId) return true;
  const [job] = await db.select({ status: jobsTable.status })
    .from(jobsTable).where(eq(jobsTable.id, plan.jobId)).limit(1);
  if (job && !isJobApprovedForInterview(job.status)) {
    logger.warn({ interviewId, jobId: plan.jobId, status: job.status }, "[interview-session-gate] blocked — work order not approved");
...
router.get("/interviews/plans", async (req: any, res) => {
  ...
  let plans: any[];
  if (caller.role === "platform_admin") {
    plans = await db.select().from(interviewPlansTable).orderBy(desc(interviewPlansTable.createdAt)).limit(MAX_PAGE_SIZE);
  } else {
    const allowedEarly = await getAllowedTenantIds(caller as any);
    if (!allowedEarly || allowedEarly.length === 0) { res.json([]); return; }
    plans = await db.select().from(interviewPlansTable)
...
router.patch("/interviews/:interviewId/recording", validate({ body: PatchRecordingBody }), async (req: any, res) => {
  const { objectPath } = req.body;
  const [session] = await db.select().from(interviewSessionsTable).where(eq(interviewSessionsTable.id, req.params.interviewId)).limit(1);
  ...
  await db.update(interviewSessionsTable).set({ recordingUrl: objectPath }).where(eq(interviewSessionsTable.id, req.params.interviewId));
  res.json({ ok: true, recordingUrl: objectPath });
});

router.get("/interviews/plans/:planId", async (req: any, res) => {
  const [plan] = await db.select().from(interviewPlansTable).where(eq(interviewPlansTable.id, req.params.planId)).limit(1);
...
router.post("/interviews/cultural-config/:jobId", validate({ body: CulturalConfigBody }), async (req, res) => {
  if (!(await gateJobForInterview(req, res, req.params.jobId))) return;
...
function buildCandidateJobContext(candidateProfile?: string, jobDescription?: string): string {
  ...
  const parts: string[] = ["---", "CONTEXT — use this to tailor every question to this specific person and role:"];
  if (candidateProfile) parts.push(`Candidate Profile:\n${candidateProfile}`);
  if (jobDescription) parts.push(`Job Description:\n${jobDescription}`);
  parts.push("---", "Reference specific skills, past experience, or job requirements in your questions. Do NOT ask generic questions — every question must feel like it was written specifically for this person applying to this role.");
  return "\n\n" + parts.join("\n\n");
}
...
const LIVE_SESSION_STATUSES = [
  "scheduled", "in_progress", "invited", "opened", "verified", "active", "paused", "resumed",
] as const;
...
function interviewMintLockKey(tenantId: string, candidateId: string, jobId: string, interviewType: string): bigint {
...
function buildGenerateLinkResponse(
  ...
  return {
    ...
    questions: (plan.questions as any[]).map((q: any) => ({
      id: q.id, text: q.text, title: q.title, category: q.category, order: q.order,
...
router.post("/interviews/generate-link", validate({ body: GenerateLinkBody }), async (req: any, res) => {
  const {
    jobId,
    candidateId = "demo",
  ...
  if (applicationId && !["direct", "pipeline"].includes(applicationId) && caller.role !== "platform_admin") {
    const [appRow] = await db.select().from(applicationsTable).where(eq(applicationsTable.id, applicationId)).limit(1);
    if (appRow) {
      const allowedTenants = await getAllowedTenantIds(caller as any);
  ...
  if (isRealCandidate && !regenerate) {
    const [existing] = await findLiveInterviewSessions(db, jobRow.tenantId as string, candidateId, jobId, interviewType);
    if (existing) {
      res.status(200).json(buildGenerateLinkResponse(existing.session, existing.plan, {
        emailSent: false, emailedTo: null, langLabel, reused: true,
      }));
      return;
    }
  }
  ...
  if (interviewType === "cultural" && !culturalDoc) {
    ...
    if (saved) {
      ...
      resolvedCustomQuestions = resolvedCustomQuestions?.length ? resolvedCustomQuestions : saved.customQuestions;
    }
  }
  const resolvedFocusDirective = (focusDirective ?? "").trim() || null;

  const { userPrompt, systemPrompt } = buildInterviewPrompt(
    interviewType, questionCount, roleContext, langLabel,
    resolvedCulturalDoc, resolvedCustomQuestions, difficulty,
    candidateProfile, jobDescription, resolvedFocusDirective ?? undefined,
  );
  ...
  const cleanCustomQuestions = (resolvedCustomQuestions ?? []).map((q) => (q ?? "").trim()).filter(Boolean);
  ...
  const mint = await db.transaction(async (tx): Promise<{ plan: any; session: any; reused: boolean }> => {
    if (isRealCandidate) {
      await tx.execute(
        sql`SELECT pg_advisory_xact_lock(${interviewMintLockKey(resolvedTenantId, candidateId, jobId, interviewType)})`,
      );
      if (regenerate) {
        const live = await findLiveInterviewSessions(tx, resolvedTenantId, candidateId, jobId, interviewType);
        if (live.length) {
          await tx.update(interviewSessionsTable)
            .set({ status: "expired", expiredAt: new Date() })
            .where(inArray(interviewSessionsTable.id, live.map((r: any) => r.session.id)));
        }
      } else {
        const [existing] = await findLiveInterviewSessions(tx, resolvedTenantId, candidateId, jobId, interviewType);
        if (existing) return { plan: existing.plan, session: existing.session, reused: true };
      }
    }

    const [p] = await tx.insert(interviewPlansTable).values({
      tenantId: resolvedTenantId, jobId, title, interviewType, language,
      questions: questions.map((q, i) => ({ ...q, id: q.id || crypto.randomUUID(), order: i + 1 })),
      culturalConfig,
      focusDirective: resolvedFocusDirective,
      estimatedDurationMinutes: interviewType === "programming" ? questionCount * 20 : questionCount * 8,
    } as any).returning();

    const [s] = await tx.insert(interviewSessionsTable).values({
  ...
  // path above).
  if (mint.reused) {
    res.status(200).json(buildGenerateLinkResponse(session, plan, {
      emailSent: false, emailedTo: null, langLabel, reused: true,
    }));
    return;
  }
  ...
  try {
    await recordCreditEvent({ tenantId: resolvedTenantId, kind: "interview", refId: session.id, metadata: { interviewType, source: "generate-link" } });
  } catch (err) {
    logger.error({ err, sessionId: session.id, tenantId: resolvedTenantId }, "[interviews] credit-event write failed (non-fatal)");
  }
...
You've been invited to complete an interview${roleLine}. It's an AI-guided session you can take whenever you're ready — no account or sign-up required.

Start your interview here:
${roomUrl}

This link is personal to you, so please don't share it. Before you begin, find a quiet spot with a working camera and microphone, and set aside about ${plan.estimatedDurationMinutes} minutes.
...
— Lexy AI Hiring Platform`;
      const result = await sendEmail({
        ...
        audit: {
          ...
          metadata: { sessionId: session.id, jobId: jobRow?.id, jobTitle: jobRow?.title, interviewType },
        },
      });
    ...
    logger.error({ err, sessionId: session.id }, "[interviews] candidate invite email failed (non-fatal)");
  }

  res.status(201).json(buildGenerateLinkResponse(session, plan, {
...
router.post("/interviews/:interviewId/submit-code", validate({ body: SubmitCodeBody }), requireSameOriginPost, requireInterviewSessionCookie, requireActiveAiConsent, async (req, res) => {
  if (!(await assertSessionJobApproved(res, req.params.interviewId))) return;
...
IMPORTANT: the "feedback" and "suggestions" fields are shown to the CANDIDATE mid-interview as iteration guidance. They must describe what works and what to improve in the code, and must NOT contain any numeric rating, grade, ranking, hiring recommendation, or overall verdict — keep all grading strictly in the score/correctness/codeQuality fields.`;
    ...
    const resp = await generateWithAI(evalPrompt, `You are a senior software engineer evaluating code quality, correctness, and efficiency. Be concise but specific. Respond with valid JSON only.\n\n${FAIRNESS_DIRECTIVE}`);
...
const GRADE_PROSE_RE = /\b(?:\d{1,3}\s*(?:\/|out of)\s*(?:10|100)|score[sd]?\s*(?:of|:|is|was)?\s*\d{1,3}|(?:rated?|rating|grade[sd]?)\s*(?:of|:|is|was|at)?\s*\d{1,3})\b/gi;
...
export function sanitizeCodeEvalForCandidate(ev: any) {
  return {
    ...
    suggestions:     Array.isArray(ev?.suggestions)
      ? ev.suggestions.map((s: unknown) => scrubGradeProse(s)).filter((s: string | null): s is string => s != null)
      : [],
...
router.post("/interviews/start", validate({ body: StartInterviewBody }), async (req, res) => {
  ...
  const [plan] = await db.select().from(interviewPlansTable).where(eq(interviewPlansTable.id, planId)).limit(1);
  if (!plan) { res.status(404).json({ error: "Plan not found" }); return; }
  ...
  if (plan.jobId) {
    ...
    if (planJob && !isJobApprovedForInterview(planJob.status)) {
      logger.warn({ planId, jobId: plan.jobId, status: planJob.status }, "[interviews/start] blocked — work order not approved");
      res.status(403).json({ error: JOB_NOT_APPROVED_MESSAGE, status: planJob.status }); return;
    }
  }
  ...
  const [session] = await db.insert(interviewSessionsTable).values({
    tenantId: plan.tenantId, applicationId, planId,
  ...
  try {
    await recordCreditEvent({ tenantId: plan.tenantId, kind: "interview", refId: session.id, metadata: { source: "interviews/start" } });
  } catch (err) {
    logger.error({ err, sessionId: session.id, tenantId: plan.tenantId }, "[interviews] credit-event write failed (non-fatal)");
  }
  ...
  void logCandidateEvent({
    ...
    applicationId: session.applicationId && !["direct", "pipeline"].includes(session.applicationId) ? session.applicationId : null,
    eventType: "INTERVIEW_STARTED",
    actorType: "candidate",
    source: "interview_agent",
    metadata: { sessionId: session.id, planId: session.planId, interviewType: (plan as any).interviewType },
  });

  res.status(201).json({ ...session, startedAt: session.startedAt?.toISOString() || null, completedAt: null, createdAt: session.createdAt.toISOString() });
});

router.post("/interviews/:interviewId/begin", beginIpLimit, beginInterviewLimit, requireSameOriginPost, async (req, res) => {
   ...
   * ──────────────────────────────────────────────────────────────────── */
  const { hasActiveAiConsent, CURRENT_AI_CONSENT_VERSION } = await import("../lib/ai-consent.js");
  const [preCheck] = await db.select({ candidateId: interviewSessionsTable.candidateId })
    .from(interviewSessionsTable)
    .where(eq(interviewSessionsTable.id, req.params.interviewId)).limit(1);
  ...
  if (!ok) {
    res.status(412).json({
      ...
      message: "Candidate has not consented to the current AI interview + biometric disclosure. The interview room surfaces the consent gate inline (GET/POST /interviews/:id/consent) before any capture begins.",
  ...
  const [updated] = Object.keys(startedAtUpdate).length > 0
    ? await db.update(interviewSessionsTable)
        .set(startedAtUpdate as any)
        .where(eq(interviewSessionsTable.id, req.params.interviewId)).returning()
    : await db.select().from(interviewSessionsTable)
        .where(eq(interviewSessionsTable.id, req.params.interviewId)).limit(1);
  const [plan] = await db.select().from(interviewPlansTable).where(eq(interviewPlansTable.id, updated.planId)).limit(1);
  const questions = (plan?.questions as any[]) ?? [];
  res.json({
    ...updated,
...
router.get("/interviews/:interviewId/consent-status", async (req, res) => {
  if (!(await assertSessionJobApproved(res, req.params.interviewId))) return;
  const [session] = await db.select({ candidateId: interviewSessionsTable.candidateId })
    .from(interviewSessionsTable)
    .where(eq(interviewSessionsTable.id, req.params.interviewId)).limit(1);
  if (!session) { res.status(404).json({ error: "not_found" }); return; }
  const { hasActiveAiConsent, getCurrentDisclosure, CURRENT_AI_CONSENT_VERSION } = await import("../lib/ai-consent.js");
...
const SessionConsentBody = z.object({
...
router.post("/interviews/:interviewId/consent", beginIpLimit, requireSameOriginPost, validate({ body: SessionConsentBody }), async (req, res) => {
  if (!(await assertSessionJobApproved(res, req.params.interviewId))) return;
  const [session] = await db.select({ candidateId: interviewSessionsTable.candidateId })
    .from(interviewSessionsTable)
    .where(eq(interviewSessionsTable.id, req.params.interviewId)).limit(1);
...
async function lookupCandidateEmail(session: any): Promise<{ email: string | null; firstName: string | null }> {
  if (session.candidateId && session.candidateId !== "demo" && session.candidateId !== "default") {
  ...
  if (session.applicationId && !["direct", "pipeline"].includes(session.applicationId)) {
    const [app] = await db.select().from(applicationsTable).where(eq(applicationsTable.id, session.applicationId)).limit(1);
    if (app?.candidateId) {
...
router.post("/interviews/:interviewId/step-up/start",
  rateLimit({ windowMs: 60_000, max: 10, scope: "stepup-start-ip" }), // per IP
  rateLimit({ windowMs: 60_000, max: 3,  scope: "stepup-start-id", keyFn: (req) => `stepup-start:${req.params.interviewId}` }), // per session
  async (req, res) => {
  const sid = req.params.interviewId;
  const [session] = await db.select().from(interviewSessionsTable).where(eq(interviewSessionsTable.id, sid)).limit(1);
  if (!session) { res.status(404).json({ error: "Not found" }); return; }
  if (session.status === "completed" || session.completedAt) { res.status(410).json({ error: "session_completed" }); return; }
  if (session.expiresAt && session.expiresAt.getTime() < Date.now()) { res.status(410).json({ error: "session_expired" }); return; }
  if ((session.stepUpAttempts ?? 0) >= STEP_UP_MAX_ATTEMPTS) {
    res.status(429).json({ error: "step_up_locked", message: "Too many failed attempts — please contact the recruiter." });
    return;
  }

  const { email, firstName } = await lookupCandidateEmail(session);
  if (!email) { res.status(409).json({ error: "no_candidate_email", message: "We don't have an email on file for this candidate. Please contact the recruiter." }); return; }
  ...
  } as any).where(eq(interviewSessionsTable.id, sid));
  ...
  res.json({ ok: true, sentTo: email.replace(/(.).+(@.+)/, "$1***$2"), expiresInMinutes: STEP_UP_OTP_TTL_MIN });
});

router.post("/interviews/:interviewId/step-up/verify",
  rateLimit({ windowMs: 60_000, max: 20, scope: "stepup-verify-ip" }), // per IP
  rateLimit({ windowMs: 60_000, max: 10, scope: "stepup-verify-id", keyFn: (req) => `stepup-verify:${req.params.interviewId}` }), // per session
  validate({ body: StepUpVerifyBody }),
  async (req, res) => {
  const sid = req.params.interviewId;
  const otp = String(req.body?.otp ?? "").trim();
  if (!/^\d{6}$/.test(otp)) { res.status(400).json({ error: "invalid_otp_format" }); return; }

  const [session] = await db.select().from(interviewSessionsTable).where(eq(interviewSessionsTable.id, sid)).limit(1);
  if (!session) { res.status(404).json({ error: "Not found" }); return; }
  if (session.status === "completed" || session.completedAt) { res.status(410).json({ error: "session_completed" }); return; }
  if (session.expiresAt && session.expiresAt.getTime() < Date.now()) { res.status(410).json({ error: "session_expired" }); return; }
  if ((session.stepUpAttempts ?? 0) >= STEP_UP_MAX_ATTEMPTS) { res.status(429).json({ error: "step_up_locked" }); return; }
  if (!session.stepUpOtpHash || !session.stepUpOtpExpiresAt || session.stepUpOtpExpiresAt.getTime() < Date.now()) {
  ...
  if (!match) {
    ...
    events.push({ kind: "step_up_otp_failed", detail: { attempts, ip: req.ip }, ts: new Date().toISOString() });
    await db.update(interviewSessionsTable)
    ...
    res.status(401).json({ error: "otp_incorrect", attemptsRemaining: Math.max(0, STEP_UP_MAX_ATTEMPTS - attempts) });
    return;
  }
  ...
  setSessionCookie(res, sid, nonce, session.expiresAt ?? new Date(Date.now() + INTERVIEW_SESSION_TTL_HOURS * 3600_000));
  void recordTrustEvent({
    sessionId: sid,
    tenantId: session.tenantId,
    candidateId: session.candidateId,
...
router.get("/interviews/:interviewId/intro", async (req: any, res) => {
  res.setHeader("Cache-Control", "no-store");
  const lexyFallback = { mode: "fallback" as const, video_url: null, fallback_image_url: null, can_skip: true, next_action: "start_lexy_interview" as const, script_text: null };
  try {
    const [session] = await db.select().from(interviewSessionsTable).where(eq(interviewSessionsTable.id, req.params.interviewId)).limit(1);
    if (!session) { res.status(404).json({ error: "Not found" }); return; }
    ...
    const planId = (session as any).planId as string | null;
    const [plan] = planId
      ? await db.select().from(interviewPlansTable).where(eq(interviewPlansTable.id, planId)).limit(1)
      : [];
    const jobId = (plan?.jobId as string | null) ?? null;
    if (!jobId) { res.json(lexyFallback); return; }

    const [job] = await db.select().from(jobsTable).where(eq(jobsTable.id, jobId)).limit(1);
    if (!job) { res.json(lexyFallback); return; }
    ...
    const resp = await resolveCandidateIntro({ jobId, language, tenantIds, preferredRecruiterUserId });
    ...
    const presignIntroAsset = async (url: string | null, allowed: RegExp): Promise<string | null> => {
    ...
    try {
      resp.video_url = await presignIntroAsset(resp.video_url, /^\/api\/storage\/objects\/recruiter-intros\//);
      ...
      logger.warn({ err, interviewId: req.params.interviewId }, "[recruiter-intro] intro asset presign failed — degrading to fallback");
    ...
    logger.warn({ err, interviewId: req.params.interviewId }, "[recruiter-intro] candidate intro resolve failed (non-fatal)");
    res.json(lexyFallback);
  }
});
...
router.get("/interviews/:interviewId", gateInterviewRead, async (req: any, res) => {
  res.setHeader("Cache-Control", "no-store");
  const session = req.interviewSession ?? (await db.select().from(interviewSessionsTable).where(eq(interviewSessionsTable.id, req.params.interviewId)).limit(1))[0];
  if (!session) { res.status(404).json({ error: "Not found" }); return; }
  ...
  if (session.candidateId && session.candidateId !== "demo" && session.candidateId !== "default") {
    const [cand] = await db.select().from(candidatesTable)
      .where(eq(candidatesTable.id, session.candidateId)).limit(1);
    if (cand) {
  ...
  const {
    bindSecret: _bs, bindFingerprint: _bf, bindUserAgent: _bua, bindIpPrefix: _bip,
    cookieNonce: _cn, stepUpOtpHash: _oh,
    ...safeSession
  } = session as any;
  ...
  const payload = isStaffRead ? safeSession : (() => {
    const {
      score: _s, answers: _a, codeSubmissions: _c,
      proctoring_events: _p, suspiciousEvents: _se,
      trustScore: _t, suspiciousEventCount: _sc,
      ...candidateSafe
    } = safeSession;
    return { ...candidateSafe, currentQuestionIndex: safeSession.currentQuestionIndex, totalQuestions: safeSession.totalQuestions };
  })();
...
router.post("/interviews/:interviewId/answer", validate({ body: AnswerBody }), requireSameOriginPost, requireInterviewSessionCookie, requireActiveAiConsent, async (req, res) => {
  if (!(await assertSessionJobApproved(res, req.params.interviewId))) return;
  ...
  if (!trimmedAnswer) {
    ...
    try {
      ...
      answers.push({ questionId, answer: answerText, score: answerScore, feedback: scoreResult.feedback });
    } catch {
  ...
  if (isComplete && session.candidateId && plan?.jobId) {
    setImmediate(() => {
      upsertIntelligenceFromInterviewSession(session.id, {
        ...
        jobId:       plan.jobId,
        candidateId: session.candidateId,
        answers:     answers.map((a: any) => ({ questionId: a.questionId, answer: a.answer, score: a.score ?? null, feedback: a.feedback })),
...
router.post("/interviews/:interviewId/proctor-event", validate({ body: ProctorEventBody }), requireSameOriginPost, requireInterviewSessionCookie, requireActiveAiConsent, async (req, res) => {
  if (!(await assertSessionJobApproved(res, req.params.interviewId))) return;
...
router.get("/interviews/:interviewId/proctor-report", async (req: any, res) => {
  ...
  const noFaceEvents   = events.filter(e => e.type === "snapshot" && e.faceVisible === false).length;
  ...
  if (noFaceEvents) violations.push(`${noFaceEvents} snapshot${noFaceEvents > 1 ? "s" : ""} with no face`);
  if (multiFace)    violations.push(`${multiFace} snapshot${multiFace > 1 ? "s" : ""} with multiple faces`);
  if (suspicious)   violations.push(`${suspicious} suspicious activity detection${suspicious > 1 ? "s" : ""}`);
  ...
  const trustEvents = await db.select().from(trustEventsTable)
  ...
  const trustLevel = effectiveTrustScore >= 85 ? "high"
                    : effectiveTrustScore >= 60 ? "medium" : "low";
  ...
  res.json({
    ...
    tabSwitches, copyEvents, pasteEvents, noFaceEvents, multiFace, suspicious,
    violations,
    events,
    trustLevel,
    /* Spec §9 fields. */
 ...
 * ─────────────────────────────────────────────────────────────────────── */
router.post("/interviews/:interviewId/upload-token", requireSameOriginPost, requireInterviewSessionCookie, requireActiveAiConsent, async (req, res) => {
  if (!(await assertSessionJobApproved(res, req.params.interviewId))) return;
  ...
  const uploadToken = issueToken({ userId: cand.userId, role: "candidate", tenantId: cand.tenantId ?? null });
  return res.json({ uploadToken });
});

router.post("/interviews/:interviewId/end", requireSameOriginPost, requireInterviewSessionCookie, requireActiveAiConsent, async (req, res) => {
  const [session] = await db.select().from(interviewSessionsTable).where(eq(interviewSessionsTable.id, req.params.interviewId)).limit(1);
  if (!session) { res.status(404).json({ error: "Not found" }); return; }
  ...
  await db.update(interviewSessionsTable).set({ status: "completed", completedAt, lastActiveAt: completedAt } as any).where(eq(interviewSessionsTable.id, req.params.interviewId));
  if (!wasAlreadyCompleted) {
    /* Resolve jobId for event (planId → plan.jobId) — best-effort, skip if unavailable. */
    const _planForEvent = session.planId
      ? await db.select({ jobId: interviewPlansTable.jobId }).from(interviewPlansTable)
          .where(eq(interviewPlansTable.id, session.planId)).limit(1).then(r => r[0] ?? null)
      : null;
    void logCandidateEvent({
      candidateId: session.candidateId,
  ...
  const PRE_COMPLETION = ["sourced", "applied", "screening", "verification", "shortlisted", "phone_screen", "assessment", "interview_scheduled", "interview"];
  ...
  if (session.applicationId && !["direct", "pipeline"].includes(session.applicationId)) {
    const [app] = await db.select().from(applicationsTable).where(eq(applicationsTable.id, session.applicationId)).limit(1);
    if (app) { connRecruiterLinked = true; connCandidateId = app.candidateId; connJobId = app.jobId ?? null; }
    ...
    const candidateApps = await db.select().from(applicationsTable)
    ...
    const targetApp = sessionJobId
      ...
      : (candidateApps.length === 1 ? candidateApps[0] : undefined);

    if (targetApp) {
      connRecruiterLinked = true; connCandidateId = targetApp.candidateId; connJobId = targetApp.jobId ?? null;
      ...
      const [sourced] = await db.select().from(sourcedCandidatesTable)
        ...
        .limit(1);
      if (sourced) {
        connRecruiterLinked = true; connCandidateId = sourced.normalizedCandidateId ?? null; connJobId = sessionJobId;
        ...
        if (PRE_COMPLETION.includes(currentStage)) {
          if (sourced.normalizedCandidateId && sessionJobId) {
            ...
            await db.update(sourcedCandidatesTable)
              .set({ rawData: { ...raw, stage: "interview_completed", interviewCompletedAt: new Date().toISOString() } })
  ...
  if (!wasAlreadyCompleted && connRecruiterLinked && connCandidateId) {
    try {
      ...
      logger.error({ err: err?.message }, "[interviews] Connection Engine completed_interview update failed");
    }
  }
  ...
  try {
    ...
    if (session.applicationId && !["direct", "pipeline"].includes(session.applicationId)) {
      ...
      if (app) {
        ...
        recipientId = cand?.createdById
          ?? job?.assignedRecruiterId
          ?? job?.assignedHiringManagerId
          ?? null;
        if (cand) {
          candidateName = `${cand.firstName ?? ""} ${cand.lastName ?? ""}`.trim() || cand.email || candidateName;
          candidateEmail = cand.email ?? null;
      ...
      let [sourced] = await db.select().from(sourcedCandidatesTable)
      ...
      candidateName = cand
        ...
        : (`${raw.firstName ?? ""} ${raw.lastName ?? ""}`.trim() || raw.name || raw.email || candidateName);
       ...
       * if no recruiter). */
      recipientId = cand?.createdById
        ?? (raw.createdById ?? raw.uploadedById ?? raw.sourcedById ?? null)
        ?? job?.assignedRecruiterId
        ?? job?.assignedHiringManagerId
        ?? null;
    }
    ...
    if (recipientId && !wasAlreadyCompleted) {
      ...
      try {
        const [recipUser] = await controlDb.select().from(usersTable).where(eq(usersTable.id, recipientId)).limit(1);
        if (recipUser?.email) {
...
— Lexy AI Hiring Platform`;
            ...
            }).catch((err) => logger.error({ err }, "Failed to send recruiter interview-completed email"));
          }
        }
      } catch (err) {
    ...
    if (candidateEmail && !wasAlreadyCompleted) {
      ...
      if (isEmailConfigured()) {
        ...
        const greetingName = candidateFirstName || candidateName.split(" ")[0] || (isArabic ? "" : "there");
  ...
  const [existingSummary] = await db.select().from(interviewSummariesTable)
    .where(eq(interviewSummariesTable.interviewSessionId, session.id))
    .orderBy(desc(interviewSummariesTable.createdAt)).limit(1);
  if (existingSummary) {
  ...
  const queued = await enqueueAiJob({
    ...
    logger.error({ sessionId: session.id, err: err?.message }, "[ai-queue] failed to enqueue summarize_interview");
    return null;
  });
  if (!queued) {
...
router.patch("/interviews/:interviewId/recruiter-comments", validate({ body: RecruiterCommentsBody }), async (req: any, res) => {
  ...
  const [updated] = await db.update(interviewSummariesTable)
    ...
    .returning();
  if (!updated) { res.status(404).json({ error: "Summary not found" }); return; }
...
router.get("/coordinator/schedules", async (req: any, res) => {
  ...
  if (caller.role !== "platform_admin") {
    ...
    if (!allowedTenants || allowedTenants.length === 0) { res.json([]); return; }
  }
...
router.post("/coordinator/schedules", validate({ body: CreateScheduleBody }), async (req: any, res) => {
  const { applicationId, interviewerId, interviewerName, location, scheduledAt, durationMinutes, type, notes } = req.body;
  ...
  const [sched] = await db.insert(interviewSchedulesTable).values({
  ...
  res.status(201).json({ ...sched, scheduledAt: sched.scheduledAt.toISOString(), createdAt: sched.createdAt.toISOString() });
});

router.put("/coordinator/schedules/:scheduleId", validate({ body: UpdateScheduleBody }), async (req: any, res) => {
  ...
  const [caller] = await controlDb.select().from(usersTable).where(eq(usersTable.id, userId)).limit(1);
  if (!caller) { res.status(401).json({ error: "Unauthorized" }); return; }
  const [existing] = await db.select().from(interviewSchedulesTable)
    .where(eq(interviewSchedulesTable.id, req.params.scheduleId)).limit(1);
  ...
  const [sched] = await db.update(interviewSchedulesTable).set(update).where(eq(interviewSchedulesTable.id, req.params.scheduleId)).returning();
  if (!sched) { res.status(404).json({ error: "Not found" }); return; }
  res.json({ ...sched, scheduledAt: sched.scheduledAt.toISOString(), createdAt: sched.createdAt.toISOString() });
});

/* ── POST /interviews/:id/feedback ── interviewer submits feedback ────────── */
router.post("/interviews/:scheduleId/feedback", validate({ body: FeedbackBody }), resolveUser, async (req, res) => {
  try {
    ...
    const [sched] = await db
      ...
      .returning();
    if (!sched) { res.status(404).json({ error: "Interview not found" }); return; }
    res.json({ id: sched.id, feedbackRating: sched.feedbackRating, feedbackNotes: sched.feedbackNotes });
  } catch (err: any) {
