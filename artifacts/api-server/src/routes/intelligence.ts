...
const VALID_ACTIONS_LIST = ["advance", "schedule", "recruiter_review", "re_engage", "manual_verification", "reject", "hold"] as const;
const VALID_OUTCOMES_LIST = ["hired", "rejected", "ghosted", "no_show", "offer_accepted", "offer_declined"] as const;
...
const OverrideBody = z.object({
  ...
  reasonDetail: z.string().trim().min(10, "A written rationale of at least 10 characters is required"),
...
async function assertJobOwnership(req: any, res: any, jobId: string): Promise<boolean> {
  ...
  if (!job) { res.status(404).json({ error: "Not found" }); return false; }
...
async function assertCandidateOwnership(req: any, res: any, candidateId: string): Promise<boolean> {
  ...
  if (!sc) { res.status(404).json({ error: "Not found" }); return false; }
...
const VALID_OUTCOMES = ["hired", "rejected", "ghosted", "no_show", "offer_accepted", "offer_declined"] as const;
const VALID_ACTIONS  = ["advance", "schedule", "recruiter_review", "re_engage", "manual_verification", "reject", "hold"] as const;
...
router.get("/job/:jobId", enforceOwnership({ kinds: ["jobId"] }), async (req, res) => {
  try {
    ...
    const records = await getIntelligenceForJob(req.params.jobId, await getDataScopeTenantIds(user));
...
router.get("/candidate/:candidateId", enforceOwnership({ kinds: ["candidateId"] }), async (req, res) => {
  try {
    const user = req.resolvedUser;
    if (!user) { res.status(401).json({ error: "Unauthorized" }); return; }
    ...
    const [cand] = await db.select({ tenantId: candidatesTable.tenantId })
      .from(candidatesTable).where(eq(candidatesTable.id, candidateId)).limit(1);
    if (!cand) { res.status(404).json({ error: "Not found" }); return; }
...
router.put("/policies/:tenantId", validate({ body: PolicyUpdateBody }), async (req, res) => {
  try {
    ...
    const { policy: policyRaw, roleId, stage, label } = req.body as {
      policy?: unknown;
      roleId?: string;
      stage?: string;
      label?: string;
...
router.get("/:jobId/:candidateId", enforceOwnership({ kinds: ["jobId", "candidateId"] }), async (req, res) => {
  try {
    const user = req.resolvedUser!;
    const record = await getIntelligenceForPair(req.params.jobId, req.params.candidateId, await getDataScopeTenantIds(user));
    if (!record) return res.status(404).json({ error: "No intelligence record found for this pair" });
...
router.get("/:jobId/:candidateId/decision", enforceOwnership({ kinds: ["jobId", "candidateId"] }), async (req, res) => {
  try {
    const user = req.resolvedUser!;
    const record = await getIntelligenceForPair(req.params.jobId, req.params.candidateId, await getDataScopeTenantIds(user));
    ...
    const agentKeys = ["screening", "sourcing", "interview", "proctoring", "outreach", "antiGhosting", "verification", "scheduling", "analytics"] as const;
...
router.post("/compute", validate({ body: ComputeIntelligenceBody }), enforceOwnership({ kinds: ["jobId", "candidateId"] }), async (req, res) => {
  try {
    const user = req.resolvedUser!;
    const { jobId, candidateId, signals } = req.body as {
      jobId?: string; candidateId?: string; signals?: Record<string, any>;
    };
...
router.post("/trigger-action", validate({ body: TriggerActionBody }), enforceOwnership({ kinds: ["jobId", "candidateId"] }), async (req, res) => {
  try {
    const { jobId, candidateId, force } = req.body as {
      jobId?: string; candidateId?: string; force?: string;
    };
    ...
    const record = await getIntelligenceForPair(jobId, candidateId, await getDataScopeTenantIds(req.resolvedUser!));
    if (!record) return res.status(404).json({ error: "No intelligence record found" });
    ...
    const policy     = await getPolicy(record.tenantId);
    const timestamps = (record.signalTimestampsJson as any) ?? {};
    const decisionResult = decideNextAction(scores, (record.signalsJson as any) ?? {}, timestamps, policy);
    ...
    const agentRun = await orchestrator.triggerAgent(trigger.agentId as any, {
      jobId,
      candidateId,
    ...
    logger.info({ action, agentId: trigger.agentId, runId: agentRun.id, jobId, candidateId }, "Action triggered from intelligence");
...
router.post("/:jobId/:candidateId/override", validate({ body: OverrideBody }), enforceOwnership({ kinds: ["jobId", "candidateId"] }), async (req, res) => {
  try {
    ...
    if (!originalDecision || !VALID_ACTIONS.includes(originalDecision as any)) {
      return res.status(400).json({ error: `originalDecision must be one of: ${VALID_ACTIONS.join(", ")}` });
    }
    if (!recruiterDecision || !VALID_ACTIONS.includes(recruiterDecision as any)) {
      return res.status(400).json({ error: `recruiterDecision must be one of: ${VALID_ACTIONS.join(", ")}` });
    }
    ...
    res.json({
      success: true,
      override: { originalDecision, recruiterDecision, recruiterReason, recruiterId, overriddenAt: new Date().toISOString() },
    });
  } catch (err: any) {
...
router.post("/:jobId/:candidateId/feedback", validate({ body: FeedbackBody }), enforceOwnership({ kinds: ["jobId", "candidateId"] }), async (req, res) => {
  try {
    ...
    logger.info({ jobId: req.params.jobId, candidateId: req.params.candidateId, rating }, "Recruiter feedback recorded");
...
router.patch("/:jobId/:candidateId/outcome", validate({ body: OutcomeBody }), enforceOwnership({ kinds: ["jobId", "candidateId"] }), async (req, res) => {
  try {
    if (!(await assertJobOwnership(req, res, req.params.jobId))) return;
    if (!(await assertCandidateOwnership(req, res, req.params.candidateId))) return;
    ...
    await recordOutcome(req.params.jobId, req.params.candidateId, outcome as typeof VALID_OUTCOMES[number]);
    ...
    logger.info({ jobId: req.params.jobId, candidateId: req.params.candidateId, outcome }, "Outcome recorded — learning updated");
