 ...
 */
import {
  searchGitHub, searchPDL, searchSerp, searchEnrichLayer,
  ...
  enrichlayer: {
    ...
    note: () =>
      process.env.ENRICH_LAYER_API_KEY
        ? (serpConfigured() ? "EnrichLayer enrichment active" : "Needs SERP_API_KEY for URL discovery")
...
function providerTimeoutMs(kind: ProviderKind): number {
  const envName = kind === "discovery"
    ? "SOURCING_DISCOVERY_TIMEOUT_MS"
    : "SOURCING_ENRICHMENT_TIMEOUT_MS";
...
export async function runSourcingProviders(
  ...
  const registry = opts.registry ?? SOURCING_PROVIDERS;
  ...
  const discoveryEntries = await Promise.all(
    discovery.map(
      async (p) =>
        [p.id, wants(p.id) ? await runProvider(p, ctx, []) : empty("Not requested")] as const,
    ),
  );
  ...
  const seedUrls = [
    ...(results.pdl?.candidates ?? []),
    ...(results.serp?.candidates ?? []),
  ]
    .map((c) => c.linkedinUrl)
  ...
  for (const p of enrichment) {
    results[p.id] = wants(p.id) ? await runProvider(p, ctx, seedUrls) : empty("Not requested");
  }
