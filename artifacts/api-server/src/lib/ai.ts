...
export const SUPPORTED_LANGUAGES: Record<string, LanguageMeta> = {
  ...
  "en-AU": {
    ...
    region: "global",
    azureLocale: "en-AU",
    azureVoice: "en-AU-NatashaNeural",
  },
  "en-IN": {
    label: "English (India)",
  ...
  "es-CL": {
    ...
    region: "global",
    azureLocale: "es-CL",
    azureVoice: "es-CL-CatalinaNeural",
  },
  "es-PE": {
    label: "Spanish (Peru)",
    nativeName: "Español (PE)",
    family: "spanish",
    speechProvider: "azure",
    llmProvider: "openai",
    region: "global",
    azureLocale: "es-PE",
    azureVoice: "es-PE-CamilaNeural",
  },
  ...
  as: {
    ...
    region: "indian",
    azureLocale: "as-IN",
    azureVoice: "as-IN-YashicaNeural",
  },
  ...
  no: {
    ...
    region: "global",
    azureLocale: "nb-NO",
    azureVoice: "nb-NO-PernilleNeural",
  },
  da: {
    label: "Danish",
    nativeName: "Dansk",
...
let directEmbeddingClient: OpenAI | null = null;
...
export async function generateEmbedding(text: string): Promise<number[] | null> {
  ...
  /* The AI-integrations proxy does NOT support POST /embeddings (400
   * "Endpoint not supported"), so embeddings must go straight to OpenAI with
   * the direct key. Fall back to the shared (proxy) client only when no
   * direct key exists — it will fail gracefully to null there. */
  const directKey = process.env.OPENAI_API_KEY;
  if (!directKey && !process.env.AI_INTEGRATIONS_OPENAI_API_KEY) {
    logger.warn("generateEmbedding: no OpenAI API key configured — skipping");
    return null;
  }
  const client = directKey
    ? (directEmbeddingClient ??= new OpenAI({ apiKey: directKey }))
    : openaiClient;
  try {
  ...
  if (llmTarget === "azure") {
    ...
    if (azure) {
      try {
        ...
        logger.warn(
          { language, err: azureErr?.message },
          "Azure OpenAI failed — falling back to OpenAI",
        );
      }
    } else {
    ...
    [
      {
        role: "system",
        content:
          systemPrompt ?? "Respond with valid JSON only — no markdown fences, no commentary.",
      },
      { role: "user", content: prompt },
    ],
    language,
  ...
  if (meta.llmProvider === "azure") {
    ...
    if (azure) {
      try {
        ...
        logger.warn(
          { language, err: err?.message },
          "Azure OpenAI chat failed — falling back to OpenAI",
        );
      }
    } else {
      logger.warn({ language }, "Azure not configured — falling back to OpenAI");
